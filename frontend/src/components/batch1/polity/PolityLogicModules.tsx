        ...
        <div className="max-w-6xl mx-auto px-6 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {logicModules.map((module) => (
                    <div
                        key={module.id}
                        className={`group relative bg-white dark:bg-[#111] rounded-2xl p-6 border-2 transition-all ${module.status === 'unlocked'
                    ...
                    >
                        <div className="flex items-start justify-between mb-4">
                            ...
                            {module.status === 'locked' && (
                                <span className="bg-gray-100 dark:bg-gray-800 text-gray-500 text-xs px-2 py-1 rounded font-bold">
                                    LOCKED 🔒
                                </span>
                            )}
                        </div>

                        <h3 className="text-lg font-bold text-[#1F2937] dark:text-white mb-2 group-hover:text-amber-600 transition-colors">
                            {module.title}
                        </h3>
