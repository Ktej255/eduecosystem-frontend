        ...
        <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {SECURITY_SYLLABUS.map((topic) => (
                    <Card key={topic.id} className="group hover:border-red-500/50 transition-all hover:shadow-lg dark:bg-[#0a0a0a] dark:border-slate-800">
                        <CardHeader className="pb-3">
                            ...
                            <CardTitle className="text-lg text-slate-900 dark:text-white group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors">
                                {topic.title}
                            </CardTitle>
                        ...
                        <CardContent>
                            ...
                            <Link href={`/student/batch1/security/${topic.id}`}>
...
function StatCard({ icon, label, value, color, border }: { icon: React.ReactNode, label: string, value: string, color: string, border: string }) {
    return (
        <div className={`bg-white dark:bg-[#0a0a0a] p-5 rounded-xl border ${border} flex items-center gap-4 shadow-sm`}>
            <div className={`w-12 h-12 rounded-lg flex items-center justify-center bg-gray-50 dark:bg-slate-900 ${color}`}>
                {icon}
            </div>
            <div>
