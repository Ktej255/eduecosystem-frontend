...
export default function GeographySectionPlanner() {
    ...
    const handleStartSession = () => {
        ...
        if (dayData.topics.includes('universe-solar-system')) {
            router.push('/student/batch1/geography/universe');
        } else if (dayData.moduleId === 'geomorphology') {
            router.push('/student/batch1/geography/geomorphology');
        } else if (dayData.moduleId === 'climatology') {
            // Assuming a generic or specific page exists, or fallback
            router.push(`/student/batch1/geography/${dayData.moduleId}`);
        } else {
            // Default fallback
            router.push(`/student/batch1/geography/${dayData.moduleId}`);
        }
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)] min-h-[600px]">
            ...
            <Card className="w-full lg:w-1/3 flex flex-col border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden bg-white dark:bg-slate-900">
                <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <Progress value={progressPercentage} className="h-2 bg-slate-100 [&>div]:bg-indigo-600" />
                    </div>
                </div>
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                            ...
                            <CardContent className="p-8 relative z-10">
                                ...
                                <div className="flex gap-3">
                                    <Button
                                        ...
                                        className="bg-white text-indigo-600 hover:bg-indigo-50 border-0 font-bold shadow-lg shadow-indigo-900/20"
                                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6">
                            <Card className="border-slate-200 shadow-sm hover:shadow-md transition-all group cursor-pointer" onClick={handleStartSession}>
                                <CardContent className="p-6 flex items-start gap-4">
                                    <div className="p-3 rounded-lg bg-green-50 text-green-600 group-hover:scale-110 transition-transform">
                                        <BookOpen className="w-6 h-6" />
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-slate-800 mb-1 group-hover:text-green-600 transition-colors">Visual Study Mode</h4>
                                        <p className="text-sm text-slate-500 mb-3">Immersive geography content with 3D models and maps.</p>
                            ...
                            <Card className="border-slate-200 shadow-sm hover:shadow-md transition-all group cursor-pointer" onClick={() => toast.info("Coming soon!")}>
                                <CardContent className="p-6 flex items-start gap-4">
                                    <div className="p-3 rounded-lg bg-amber-50 text-amber-600 group-hover:scale-110 transition-transform">
                                        <Target className="w-6 h-6" />
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-slate-800 mb-1 group-hover:text-amber-600 transition-colors">Practice MCQs</h4>
                                        <p className="text-sm text-slate-500 mb-3">Test your understanding of {dayData.title}.</p>
