...
export default function EthicsDashboard() {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-500">
            ...
            {/* View Toggle */}
            <div className="flex justify-center border-b border-gray-200 dark:border-gray-800 pb-1">
                <div className="flex gap-4">
                    <button
                        onClick={() => setViewMode('theory')}
                        className={`px-6 py-2 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${viewMode === 'theory' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                    >
                        <Brain className="w-4 h-4" /> Core Theory
                    </button>
                    <button
                        onClick={() => setViewMode('case-studies')}
                        className={`px-6 py-2 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 ${viewMode === 'case-studies' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                    >
            ...
            {viewMode === 'theory' ? (
                        ...
                        <div key={topic.id} className="group bg-white dark:bg-[#111] rounded-2xl border border-gray-200 dark:border-gray-800 p-6 hover:border-indigo-300 transition-all hover:shadow-md">
                            <div className="pb-3">
                                <div className="flex justify-between items-start">
                                    <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300`}>
                                        {topic.category}
                                    </span>
                ...
                <div className="text-center py-20 bg-gray-50 dark:bg-[#111] rounded-3xl border border-gray-200 dark:border-gray-800 animate-in zoom-in-95 duration-300">
                    <div className="max-w-md mx-auto">
                        <Scale className="w-16 h-16 text-indigo-500 mx-auto mb-6" />
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Internal Case Study Simulator</h2>
...
function StatCard({ icon, label, value, color }: { icon: React.ReactNode, label: string, value: string, color: string }) {
    return (
        <div className="bg-white dark:bg-[#111] p-5 rounded-2xl border border-gray-200 dark:border-gray-800 flex items-center gap-4">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center bg-gray-50 dark:bg-[#1a1a1a] ${color}`}>
                {icon}
            </div>
            <div>
