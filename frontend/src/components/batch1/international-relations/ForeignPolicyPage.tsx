...
import { ArrowLeft, Flag, BookOpen } from "lucide-react";
import Link from 'next/link';
import { FOREIGN_POLICY_CONTENT } from './data/foreign-policy-content';
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-bold flex items-center gap-2 text-indigo-900 dark:text-indigo-100">
                                ...
                                {FOREIGN_POLICY_CONTENT.title}
                            </h1>
                            <p className="text-xs text-indigo-500 dark:text-indigo-400">{FOREIGN_POLICY_CONTENT.description}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-1 space-y-4">
                    <Card className="sticky top-24 border-indigo-100 dark:border-indigo-900">
                        <CardHeader>
                            <CardTitle className="text-sm uppercase tracking-wider text-indigo-500">Key Doctrines</CardTitle>
                        </CardHeader>
                ...
                <div className="md:col-span-2 space-y-8">
                    {FOREIGN_POLICY_CONTENT.sections.map(sec => (
                        <section key={sec.id} id={sec.id} className="scroll-mt-24">
                            <Card className="overflow-hidden border-t-4 border-t-indigo-500 border-x-0 border-b-0 shadow-sm hover:shadow-md transition-shadow">
