    ...
    return (
        <div className="space-y-8" style={{ fontSize: `${fontSize}px` }}>
            <div className="border-b-2 border-orange-800 pb-4 mb-8">
                ...
                <p className="text-gray-600 font-medium italic">{mainSubtitle}</p>
            </div>
            ...
            {data.sections.map((section, sidx) => (
                <div key={sidx} className="mb-10">
                        ...
                        <div className="space-y-4">
                            {section.content.map((item, cidx) => {
                                if (typeof item === 'string') {
                                    return <p key={cidx} className="leading-relaxed text-gray-800">{item}</p>;
                                }
                                return (
                                    <p key={cidx} className="leading-relaxed text-gray-800">
                                        ...
                                        {item.text}
                                        {item.note && <span className="text-gray-500 italic ml-1">({item.note})</span>}
                                    </p>
                                );
                            })}
                        </div>
                                ...
                                <thead className="bg-gray-50">
                                    <tr>
                                        {section.s6TableHeaders.map((h: string, i: number) => (
                                            <th key={i} className="p-3 border font-bold text-gray-700">{h}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                    ...
                    {section.footer && (
                        <div className="mt-4 bg-gray-50 p-4 rounded-lg border-l-4 border-gray-300 italic text-gray-700 text-sm">
            ...
            <div className="text-center mt-12 pt-8 border-t border-dashed border-gray-300">
                <p className="text-gray-400 italic text-lg">{data.end}</p>
            </div>
        </div>
    );
}
