        ...
        <div className="min-h-screen bg-slate-50 dark:bg-black p-4 md:p-8 text-slate-900 dark:text-slate-100">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500">Visual mastery of high-yield geographic locations.</p>
                        </div>
                    </div>

                    <div className="flex gap-2 bg-white dark:bg-slate-900 p-1 rounded-lg border border-slate-200 dark:border-slate-800 overflow-x-auto">
                        <Button
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 h-[70vh]">
                    {/* Map Area */}
                    <Card className="md:col-span-3 bg-slate-100 dark:bg-slate-900 border-slate-200 dark:border-slate-800 p-1 relative overflow-hidden group">
                        <IndiaMapViz activeLayer={activeLayer} />
                    </Card>

                    {/* Sidebar Details */}
                    <Card className="md:col-span-1 bg-white dark:bg-[#0a0a0a] border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden">
                        <div className="p-4 border-b border-slate-100 dark:border-slate-800 font-bold flex items-center gap-2 bg-slate-50 dark:bg-slate-900/50">
                        ...
                        <div className="flex-1 p-4 overflow-y-auto space-y-3 custom-scrollbar">
                            ...
                            {sidebarItems.length === 0 && (
                                <p className="text-center text-xs text-slate-400 mt-4">No data available for this layer.</p>
                            )}
                        </div>
                    </Card>
                </div>
...
function LocationItem({ name, state, tag }: { name: string, state: string, tag: string }) {
    return (
        <div className="p-3 rounded-lg border border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-900/50 cursor-pointer transition-colors group">
