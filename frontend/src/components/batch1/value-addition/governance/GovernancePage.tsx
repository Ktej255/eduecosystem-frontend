            ...
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-500">Ministry-wise Schemes & Policies Database.</p>
                        </div>
                    </div>
                    <div className="relative w-full md:w-96">
                        ...
                        <Input
                            ...
                            className="pl-10 bg-white dark:bg-slate-900 focus-visible:ring-amber-500"
                            value={search}
                ...
                <div className="space-y-4">
                    {filteredMinistries.map(ministry => (
                        <div key={ministry.id} className="bg-white dark:bg-[#0a0a0a] border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
                            <div
                                className="p-6 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors flex justify-between items-center"
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500 uppercase tracking-widest">{ministry.minister}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                            ...
                            <AnimatePresence>
                                {activeMinistry === ministry.id && (
                                    <motion.div
                                        ...
                                        className="border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-[#111]"
                                    >
                                        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {ministry.schemes.map(scheme => (
                                                <Card key={scheme.id} className="border-slate-200 dark:border-slate-800 hover:shadow-md transition-all">
                                                    ...
                                                    <CardContent className="text-sm space-y-2">
                                                        <p className="text-slate-600 dark:text-slate-300">{scheme.description}</p>
                                                        <div className="grid grid-cols-2 gap-2 text-xs text-slate-500 mt-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                                                            <div>YEAR: <span className="font-mono text-slate-700 dark:text-slate-200">{scheme.launchYear}</span></div>
                                                            <div>BUDGET: <span className="font-mono text-slate-700 dark:text-slate-200">{scheme.budget}</span></div>
                                                            <div className="col-span-2">BENEFICIARIES: <span className="font-mono text-slate-700 dark:text-slate-200">{scheme.beneficiaries}</span></div>
                                                        </div>
                                                    </CardContent>
                                                </Card>
                                            ))}
