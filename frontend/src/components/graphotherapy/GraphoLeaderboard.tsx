...
export default function GraphoLeaderboard() {
    ...
    if (loading) return <div className="text-center p-4 text-gray-500">Loading rankings...</div>;

    return (
        <div className="bg-white dark:bg-neutral-800/50 backdrop-blur-xl rounded-2xl border border-gray-100 dark:border-white/10 overflow-hidden shadow-xl">
            {/* Header */}
            <div className="p-4 bg-gradient-to-r from-amber-500 to-orange-600 text-white flex items-center justify-between">
                ...
                <div className="text-xs bg-white/20 px-2 py-0.5 rounded-full">Global</div>
            </div>
                ...
                {entries.map((entry, index) => (
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <div className={`font-semibold text-sm truncate ${entry.is_current_user ? 'text-amber-900 dark:text-amber-200' : 'text-gray-900 dark:text-gray-200'}`}>
                                {entry.user_name}
