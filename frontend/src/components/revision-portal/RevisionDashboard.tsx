...
import {
    ...
    RevisionProgress,
    StudyStreak
} from '@/components/batch1/polity/revision/progress-utils';
// Note: We are temporarily importing utils from batch1, 
// eventually we should move progress-utils to a shared location too.
...
export default function GenericRevisionDashboard({
    ...
    const stats = useMemo(() => {
        ...
        const totalFlashcards = chapters.reduce((sum, ch) => sum + (ch.flashcards?.length || 0), 0);
        const completedFlashcards = Object.values(progress).reduce((sum, p) => sum + p.flashcardsCompleted, 0);
        const totalMcqs = chapters.reduce((sum, ch) => sum + (ch.mcqs?.length || 0), 0);
        ...
        return {
            ...
            totalMcqs,
            completedMcqs,
            overallProgress: totalFlashcards + totalMcqs > 0 ? Math.round(((completedFlashcards + completedMcqs) / (totalFlashcards + totalMcqs)) * 100) : 0
        };
    }, [progress, chapters]);
    ...
    const startCustomSession = () => {
        ...
        window.location.href = `${basePath}/custom?chapters=${chapterIds}`;
    };
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-[#030303] dark:via-[#050510] dark:to-[#030303] pb-20">
            ...
            <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white pt-12 pb-24 px-6 relative overflow-hidden">
                ...
                <div className="max-w-6xl mx-auto relative z-10">
                    <Link href={backLink} className="inline-flex items-center gap-2 text-indigo-200 hover:text-white transition-colors mb-8 text-sm font-medium">
            ...
            {/* Custom Session Panel */}
            {
                showCustomSession && (
                    <div className="max-w-6xl mx-auto px-6 mt-6">
                        <div className="bg-card dark:bg-[#111] rounded-2xl border border-border p-6 shadow-lg">
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            className="bg-card dark:bg-[#111] border border-border rounded-xl px-3 py-2 text-sm"
                        >
                            ...
                            >
                                <div className="flex items-start justify-between mb-3">
                                    <div>
                                        ...
                                        <div className="font-bold text-foreground group-hover:text-indigo-600 transition-colors">
                                            {ch.title}
                                        </div>
                                    </div>
                                    {p?.mastered && (
