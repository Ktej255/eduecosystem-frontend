...
export default function AIAssistant() {
    ...
    const [messages, setMessages] = useState<{ role: string, text: string }[]>([
        { role: "ai", text: "Hello! I'm your holistic learning assistant. How can I help you today?" }
    ])
    ...
    return (
        <>
            ...
            {/* Chat Window */}
            <div className={`fixed bottom-8 right-8 w-96 bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl flex flex-col transition-all duration-300 z-50 overflow-hidden ${isOpen ? "h-[500px] opacity-100 translate-y-0" : "h-0 opacity-0 translate-y-10 pointer-events-none"}`}>
                {/* Header */}
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-gray-700">
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                            <div className={`max-w-[80%] p-3 rounded-xl text-sm ${msg.role === "user" ? "bg-cyan-600 text-white rounded-tr-none" : "bg-gray-800 text-gray-200 rounded-tl-none"}`}>
                                {msg.text}
                            </div>
                        </div>
                    ))}
