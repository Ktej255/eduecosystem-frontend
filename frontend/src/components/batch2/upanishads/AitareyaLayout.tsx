    ...
    return (
        <div className="space-y-16">
            <div className="text-center max-w-4xl mx-auto space-y-6">
                <h2 className="text-5xl md:text-7xl font-serif font-black text-emerald-100 italic tracking-tight">
                    {lang === "en" ? "The Masterpiece of Creation" : "सृष्टि की उत्कृष्ट कृति"}
                </h2>
                <p className="text-emerald-400/60 leading-relaxed text-xl max-w-2xl mx-auto font-light">
                    {lang === "en"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {chapters.map((c, i) => (
                    <motion.div
                        ...
                        className="bg-slate-900 border border-emerald-500/10 rounded-[2.5rem] p-8 text-center hover:border-emerald-500/40 hover:bg-emerald-500/5 transition-all cursor-pointer group"
                    >
                        ...
                        <p className="text-emerald-200/40 text-[10px] leading-relaxed font-black uppercase tracking-widest">{c.desc}</p>
                    </motion.div>
                ))}
            </div>

            <div className="bg-slate-950/80 border border-emerald-500/20 rounded-[3rem] p-12 text-center relative overflow-hidden group backdrop-blur-md">
                <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/5 via-transparent to-emerald-500/5" />
                <p className="text-sky-500/40 text-[10px] font-black uppercase tracking-[0.6em] mb-6">The Realization of Vamadeva</p>
                <div className="space-y-4">
                    <p className="text-4xl md:text-5xl font-serif text-emerald-50 italic">"I broke from the cage like a hawk!"</p>
                    <p className="text-emerald-400/60 text-lg font-light leading-relaxed max-w-3xl mx-auto">
                        {lang === "en"
    ...
    return (
        <div className="space-y-12">
            <div className="text-center space-y-4">
                <h2 className="text-3xl md:text-6xl font-serif font-black text-emerald-100 tracking-tighter">{title}</h2>
                <div className="flex items-center justify-center gap-6">
                    <div className="h-px w-20 bg-gradient-to-r from-transparent to-emerald-500/30" />
                    <p className="text-emerald-400/60 font-black tracking-[0.5em] uppercase text-[10px]">{subtitle}</p>
            ...
            >
                <div className="bg-slate-900 border border-emerald-500/30 rounded-[4rem] overflow-hidden shadow-[0_0_120px_rgba(16,185,129,0.1)] backdrop-blur-xl">
                    <div className="p-8 md:p-16 space-y-12">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-emerald-400/50 text-[10px] font-black uppercase tracking-[0.3em] mt-1">
                                        Ch {shloka.chapter} • Sec {shloka.section} • Verse {shloka.verse}
                                    </p>
                                </div>
                            </div>
                            <div className="flex items-center gap-6 bg-slate-950/50 p-3 rounded-2xl border border-white/5 shadow-inner">
                                <button
                                    onClick={() => setCurrentIndex(prev => Math.max(0, prev - 1))}
                                    disabled={currentIndex === 0}
                                    className="p-3 text-emerald-400 hover:bg-emerald-500 hover:text-white rounded-xl transition-all disabled:opacity-20"
                                >
                                    <ChevronLeft className="w-8 h-8" />
                                </button>
                                <span className="text-emerald-100 font-bold text-lg">{currentIndex + 1} / {data.length}</span>
                                <button
                        ...
                        <div className="bg-slate-950/80 rounded-[3rem] p-12 md:p-16 border border-emerald-500/10 shadow-inner group relative overflow-hidden">
                            <div className="absolute -top-10 -right-10 w-40 h-40 bg-emerald-500/5 rounded-full blur-[80px]" />
                            <p className="text-3xl md:text-5xl text-emerald-50 leading-[1.7] text-center font-serif italic" style={{ fontFamily: "'Noto Serif Devanagari', serif" }}>
                                {shloka.sanskrit}
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="space-y-6">
                                <div className="p-10 bg-emerald-900/10 rounded-[2.5rem] border border-emerald-500/10 h-full relative group">
                                    <Languages className="absolute top-8 right-8 w-6 h-6 text-emerald-500/20 group-hover:text-emerald-500/40 transition-colors" />
                                    <h4 className="text-emerald-400/60 text-[10px] font-black uppercase tracking-widest mb-6">Translation</h4>
                                    <p className="text-emerald-50 text-2xl leading-relaxed italic font-light">
                                        {lang === "en" ? shloka.english : shloka.hindi}
                                    </p>
                                </div>
                            </div>
                            <div className="space-y-6">
                                <div className="p-12 bg-gradient-to-br from-emerald-600/30 to-teal-600/30 rounded-[3rem] border border-emerald-400/30 shadow-2xl relative overflow-hidden h-full flex flex-col justify-between">
                                    <div className="absolute -top-12 -right-12 w-48 h-48 bg-emerald-500/10 rounded-full blur-[100px]" />
                                    <div>
                                        <h4 className="text-emerald-200 font-black mb-8 flex items-center gap-4 text-xl tracking-tight">
                                        ...
                                        <p className="text-emerald-50 text-2xl md:text-3xl leading-relaxed font-serif italic text-center pb-10 border-b border-emerald-400/20">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mt-10">
                                        <button
                                            ...
                                            className="bg-white text-emerald-900 hover:bg-emerald-50 font-black p-5 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-2xl hover:scale-[1.03] active:scale-95"
                                        >
                                            <Flame className="w-6 h-6" /> SADHANA
                                        </button>
                                        <button
                                            onClick={() => (window as any).showExperienceReport && (window as any).showExperienceReport()}
                                            className="bg-emerald-900/40 text-emerald-200 border border-emerald-400/20 hover:bg-slate-800 font-bold p-5 rounded-2xl flex items-center justify-center gap-3 transition-all"
                                        >
                        ...
                        {shloka.wordMeanings && shloka.wordMeanings.length > 0 && (
                            <div className="p-12 bg-slate-950/40 rounded-[3rem] border border-white/5">
                                <h4 className="text-emerald-600 text-[10px] font-black uppercase tracking-widest mb-10 flex items-center gap-2">
                                    <Activity className="w-4 h-4" /> Cosmic Codes
                                </h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    {shloka.wordMeanings.map((m: any, i: number) => (
                                        <div key={i} className="flex flex-col gap-2 p-6 bg-slate-900/50 rounded-2xl border border-emerald-500/5 hover:border-emerald-500/40 transition-all hover:bg-slate-900">
                                            <span className="text-emerald-100 font-serif text-2xl">{m.devanagari}</span>
                                            <span className="text-emerald-400/40 text-[10px] uppercase font-black tracking-tight">{m.sanskrit}</span>
                                            <span className="text-emerald-200/60 text-sm mt-2">{lang === "en" ? m.english : m.hindi}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
    ...
    if (mode === 'immersive') {
        return (
            <div className="relative w-full min-h-screen bg-black">
                ...
                <AitareyaImmersiveExperience lang={lang} />
            </div>
        );
    }

    return (
        <div className="min-h-screen bg-slate-950 text-emerald-50 selection:bg-emerald-500/30">
            ...
            <header className="relative h-[65vh] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 text-center px-6 max-w-5xl">
                    <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                        <span className="inline-block px-10 py-3 bg-emerald-500/10 text-emerald-400 rounded-full text-[10px] font-black tracking-[0.7em] uppercase border border-emerald-500/20 backdrop-blur-md mb-12">
                            Rig Veda • The Source of Awareness
                        </span>
                        <h1 className="text-7xl md:text-[12rem] font-serif font-black text-transparent bg-clip-text bg-gradient-to-b from-emerald-100 via-emerald-400 to-emerald-950 leading-none tracking-tighter mb-8 shadow-2xl">
                            Aitareya
                        </h1>
                        <div className="flex flex-col md:flex-row items-center justify-center gap-6 mt-12 text-3xl md:text-4xl font-light text-emerald-300/40 font-serif">
            ...
            <nav className="sticky top-0 z-50 bg-slate-950/90 backdrop-blur-3xl border-b border-emerald-900/20 px-6 py-6">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        <button onClick={() => router.push("/student/batch2/upanishads")} className="p-4 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500 hover:text-white rounded-3xl transition-all border border-emerald-500/20 shadow-xl">
                            <ArrowLeft className="w-6 h-6" />
                        </button>
                        <div className="hidden sm:block">
                            <h2 className="text-2xl font-black text-white leading-tight font-serif tracking-tight">Aitareya</h2>
                            <p className="text-emerald-600/60 text-[10px] font-black uppercase tracking-widest">{lang === "en" ? "The Observer" : "प्रज्ञा"}</p>
                        </div>
                    </div>

                    <div className="flex bg-slate-900/80 rounded-[2rem] p-2 border border-white/5 shadow-2xl overflow-x-auto no-scrollbar max-w-[60%] md:max-w-none">
                        {tabs.map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id as any)}
                                className={`flex items-center gap-3 px-8 py-4 rounded-[1.5rem] text-xs font-black transition-all ${activeTab === tab.id ? "bg-emerald-50 text-emerald-900 shadow-2xl scale-105" : "text-emerald-600 hover:text-emerald-400"}`}
                            >
                    ...
                    <div className="flex items-center gap-6">
                        <div className="flex bg-slate-900/80 rounded-2xl p-1.5 border border-white/5">
                            {(["en", "hi"] as const).map((l) => (
                                <button
                                    key={l}
                                    onClick={() => setLang(l)}
                                    className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all ${lang === l ? "bg-emerald-500 text-white" : "text-emerald-600 hover:text-emerald-400"}`}
                                >
                                    {l.toUpperCase()}
                                </button>
                            ))}
            ...
            <footer className="bg-slate-950 py-56 text-center border-t border-emerald-900/10 relative overflow-hidden">
                <div className="relative z-10 space-y-16 px-6">
                    <div className="text-9xl md:text-[12rem] font-serif text-emerald-500/10 animate-pulse">प्रज्ञानं ब्रह्म</div>
                    <div className="space-y-6">
                        <p className="text-6xl md:text-8xl font-serif text-emerald-50 leading-none tracking-tighter">सत्यमेव जयते</p>
                        <p className="text-emerald-400/60 italic text-2xl max-w-3xl mx-auto font-light leading-relaxed">
                    ...
                    <div className="pt-24">
                        <p className="text-emerald-500/40 text-[10px] font-black uppercase tracking-[0.8em]">— Aitareya Upanishad —</p>
                    </div>
                </div>
            ...
            <AnimatePresence>
                {sadhanaActive && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/90 backdrop-blur-2xl" onClick={() => setSadhanaActive(false)} />
