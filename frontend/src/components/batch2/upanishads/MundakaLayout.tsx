...
function MundakaOverview({ lang, setActiveTab }: { lang: "en" | "hi", setActiveTab: (tab: any) => void }) {
    const highlights = [
        ...
        },
        {
            ...
            emoji: "🐦",
            color: "from-green-400 to-[var(--sp-bg)]",
            tab: "mundaka3"
        },
        {
    ...
    return (
        <div className="space-y-16">
            ...
            <div className="bg-gradient-to-r from-[var(--sp-gold)]/20 to-[var(--sp-bg)]/20 rounded-[2.5rem] p-12 border border-[var(--sp-border)] text-center relative overflow-hidden group">
                ...
                <p className="text-[var(--sp-text-hi)]0/60 text-sm uppercase tracking-[0.3em] font-black mb-6">The Immortal Declaration</p>
                <h3 className="text-5xl md:text-7xl font-serif text-[var(--sp-text-hi)] mb-6 drop-shadow-2xl">सत्यमेव जयते</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {highlights.map((h, i) => (
                    <motion.div
                        ...
                        className="bg-[var(--sp-surface)] border border-[var(--sp-border)] rounded-3xl p-8 text-center hover:bg-slate-800 transition-all cursor-pointer group"
                    ...
                    >
                        ...
                        <p className="text-[var(--sp-text-hi)] text-sm">{h.desc}</p>
                    </motion.div>
                ))}
            </div>
        </div>
    ...
    return (
        <div className="space-y-8">
            ...
            <motion.div
                ...
                className="bg-[var(--sp-surface)] rounded-[2.5rem] border border-[var(--sp-border)] overflow-hidden shadow-2xl p-8 md:p-12 space-y-10"
            >
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[var(--sp-text-lo)] text-xs font-bold uppercase tracking-widest">Mundaka {shloka.mundaka} • Khanda {shloka.khanda}</p>
                        </div>
                    </div>
                </div>

                <div className="bg-[var(--sp-surface)]/60 rounded-3xl p-10 border border-[var(--sp-border)] shadow-inner">
                    <p className="text-3xl md:text-4xl text-[var(--sp-text-hi)] text-center leading-relaxed font-serif" style={{ fontFamily: "'Noto Sans Devanagari', serif" }}>
                        {shloka.sanskrit}
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="space-y-6">
                        <div className="bg-[var(--sp-surface)] rounded-2xl p-6 border border-[var(--sp-border)] h-full">
                            ...
                            <p className="text-[var(--sp-text-hi)] text-xl leading-relaxed font-light italic">
                    ...
                    <div className="bg-gradient-to-br from-[var(--sp-gold)]/20 to-[var(--sp-bg)]/20 rounded-2xl p-8 border border-[var(--sp-border)] shadow-xl relative overflow-hidden group">
                        ...
                        <p className="text-[var(--sp-text-hi)] text-xl leading-relaxed font-medium relative z-10">
                ...
                {shloka.wordMeanings && shloka.wordMeanings.length > 0 && (
                    <div className="pt-4">
                        <h4 className="text-[var(--sp-gold)] text-[10px] font-black uppercase tracking-[0.3em] flex items-center gap-2 mb-6">
                            <BookOpen className="w-3 h-3" />
                            {lang === "en" ? "Key Concepts" : "मुख्य अवधारणाएँ"}
                        </h4>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {shloka.wordMeanings.map((m: any, i: number) => (
                                <div key={i} className="bg-[var(--sp-surface)]/50 border border-[var(--sp-border)] rounded-2xl p-4 hover:border-[var(--sp-border)] transition-all">
                                    <p className="text-[var(--sp-text-hi)] font-serif text-xl mb-1">{m.devanagari}</p>
                                    <p className="text-[var(--sp-gold)] text-[10px] uppercase tracking-tighter mb-2 font-bold">{m.sanskrit}</p>
                ...
                <div className="flex flex-wrap gap-4 pt-4">
                    <button
                        ...
                        className="flex-1 bg-emerald-600/20 hover:bg-[#1a1a33] border border-[var(--sp-border)] rounded-2xl p-6 text-[var(--sp-text-hi)] font-bold text-lg flex items-center justify-center gap-3 transition-all group"
                    >
                ...
                <div className="flex bg-[var(--sp-surface)]/50 p-2 rounded-full border border-[var(--sp-border)] gap-2 overflow-x-auto no-scrollbar max-w-xs md:max-w-none">
                    {data.map((_, i) => (
                        <button
                            ...
                            className={`w-10 h-10 rounded-full text-sm font-black transition-all ${i === currentIndex ? 'bg-emerald-500 text-slate-950' : 'text-[var(--sp-text-hi)]0/40 hover:text-[var(--sp-text-lo)]'}`}
                        >
                            {i + 1}
                        </button>
                    ))}
    ...
    if (mode === 'immersive') {
        return (
            <div className="relative w-full min-h-screen bg-[var(--sp-bg)]">
                ...
                <MundakaImmersiveExperience lang={lang} onClose={() => setMode('classic')} />
            </div>
        );
    }

    return (
        <div className="min-h-screen bg-[var(--sp-surface)] text-white selection:bg-emerald-500/30 selection:text-[var(--sp-text-hi)]">
            ...
            <header className="relative h-[60vh] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 text-center px-6 max-w-5xl">
                    <motion.div
                        ...
                        className="space-y-8"
                    >
                        <span className="inline-block px-6 py-2 bg-emerald-500/10 text-[var(--sp-text-lo)] rounded-full text-[10px] font-black tracking-[0.5em] uppercase border border-[var(--sp-border)] backdrop-blur-md">
                ...
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="hidden sm:block">
                            ...
                            <p className="text-[var(--sp-gold)]/60 text-[10px] font-black uppercase tracking-widest">{lang === "en" ? "The Shaven-Headed Upanishad" : "मुण्डित ज्ञान उपनिषद"}</p>
                        </div>
                    </div>

                    <div className="flex bg-[var(--sp-surface)]/80 rounded-[1.25rem] p-1.5 border border-white/5 shadow-2xl">
                        {tabs.map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id as any)}
                                className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === tab.id ? "bg-emerald-50 shadow-lg shadow-emerald-500/20 text-slate-950" : "text-[var(--sp-gold)] hover:text-[var(--sp-text-lo)]"}`}
                            >
                        ...
                        <div className="flex bg-[var(--sp-surface)]/50 rounded-xl p-1 border border-white/5">
                            {(["en", "hi"] as const).map((l) => (
                                <button
                                    key={l}
                                    onClick={() => setLang(l)}
                                    className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${lang === l ? "bg-emerald-500 text-slate-950" : "text-[var(--sp-gold)] hover:text-[var(--sp-text-lo)]"}`}
                                >
                                    {l.toUpperCase()}
                                </button>
                            ))}
            ...
            <footer className="group bg-[var(--sp-surface)] py-32 text-center border-t border-[var(--sp-border)] relative overflow-hidden">
                <div className="relative z-10 space-y-8">
                    <p className="text-4xl md:text-6xl font-serif text-[var(--sp-text-hi)] leading-tight transition-colors group-hover:text-[var(--sp-text-lo)]">भिद्यते हृदयग्रन्थिश्छिद्यन्ते सर्वसंशयाः</p>
                    ...
                    <p className="text-[var(--sp-text-hi)]0/40 text-[10px] font-black uppercase tracking-[0.4em] mt-12">— Mundaka Upanishad 2.2.8 —</p>
                </div>
