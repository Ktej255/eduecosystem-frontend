...
function ChandogyaOverview({ lang, setActiveTab }: { lang: "en" | "hi", setActiveTab: (tab: any) => void }) {
    const analogies = [
        { title: lang === "en" ? "Clay Analogy" : "मिट्टी उपमा", desc: lang === "en" ? "All pots are just clay" : "सब घड़े केवल मिट्टी हैं", emoji: "🏺", color: "from-[var(--sp-gold)] to-[var(--sp-bg)]", id: 2 },
        { title: lang === "en" ? "Banyan Seed" : "बरगद बीज", desc: lang === "en" ? "Invisible essence creates huge tree" : "अदृश्य सार से विशाल वृक्ष", emoji: "🌳", color: "from-[var(--sp-gold)] to-green-500", id: 7 },
        { title: lang === "en" ? "Salt Water" : "नमक पानी", desc: lang === "en" ? "Brahman pervades everything" : "ब्रह्म सब में व्याप्त है", emoji: "🧂", color: "from-cyan-400 to-[var(--sp-bg)]", id: 9 },
        { title: lang === "en" ? "Blindfolded Man" : "आंखों पर पट्टी", desc: lang === "en" ? "You need a Guru" : "तुम्हें गुरु चाहिए", emoji: "🧭", color: "from-[var(--sp-gold)] to-[var(--sp-bg)]", id: 11 },
    ];

    return (
        <div className="space-y-20">
            <div className="text-center max-w-4xl mx-auto space-y-6">
                <span className="text-[var(--sp-text-hi)]0 font-black uppercase tracking-[0.4em] text-[10px]">Pillars of Oneness</span>
                ...
                <div className="w-24 h-1 bg-gradient-to-r from-[var(--sp-gold)] to-cyan-500 mx-auto rounded-full" />
            ...
            <div className="relative group overflow-hidden rounded-[3rem] border border-[var(--sp-border)]">
                <div className="absolute inset-0 bg-gradient-to-br from-[var(--sp-gold)]/20 via-cyan-950/40 to-slate-950 group-hover:scale-105 transition-transform duration-1000" />
                <div className="relative z-10 p-12 md:p-20 text-center space-y-8">
                    <p className="text-[var(--sp-text-hi)]0/60 text-sm uppercase tracking-[0.5em] font-black">The Ultimate Declaration</p>
                    <motion.h3
                    ...
                    <div className="pt-8 flex justify-center">
                        ...
                        >
                            <div className="absolute inset-0 bg-gradient-to-r from-[var(--sp-gold)] to-cyan-500 opacity-0 group-hover:opacity-100 transition-opacity" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {analogies.map((a, i) => (
                    ...
                    >
                        ...
                        <p className="text-[var(--sp-text-hi)] text-sm leading-relaxed">{a.desc}</p>
                    </motion.div>
                ))}
            </div>
        </div>
    ...
    return (
        <div className="space-y-12">
            <div className="text-center space-y-4">
                <span className="text-[var(--sp-text-hi)]0 font-black uppercase tracking-[0.5em] text-[10px]">Wisdom Stream</span>
                ...
                <div className="p-8 md:p-16 space-y-12">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-6">
                            <div className="w-16 h-16 rounded-[1.5rem] bg-gradient-to-br from-[var(--sp-gold)] to-cyan-500 flex items-center justify-center text-3xl font-black text-slate-950 shadow-[0_0_30px_rgba(20,184,166,0.4)]">
                                {shloka.id}
                            </div>
                            <div>
                                <h3 className="text-3xl font-black text-white tracking-tight italic">{shloka.theme || "Verse"}</h3>
                                <p className="text-[var(--sp-text-lo)] text-xs font-black uppercase tracking-widest mt-1">Sadvidya • Ch {shloka.chapter}, Sec {shloka.section}</p>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        ...
                        <div className="bg-gradient-to-br from-[var(--sp-gold)]/10 to-transparent rounded-3xl p-10 border border-[var(--sp-border)] shadow-xl relative overflow-hidden group">
                            <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-30 transition-opacity">
                                <Sparkles className="w-16 h-16 text-[var(--sp-text-lo)]" />
                            </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-6">
                        <button
                            ...
                            className="flex-1 bg-teal-600 hover:bg-[#1a1a33] rounded-3xl p-8 text-slate-950 font-black text-xl flex items-center justify-center gap-4 transition-all hover:scale-[1.02] active:scale-[0.98] shadow-[0_20px_40px_rgba(20,184,166,0.3)]"
                        >
            ...
            <div className="flex items-center justify-center gap-10">
                <button
                    ...
                    className="w-20 h-20 flex items-center justify-center bg-[var(--sp-surface)] text-[var(--sp-text-lo)] rounded-full border border-[var(--sp-border)] hover:bg-[#1a1a33] hover:text-slate-950 transition-all disabled:opacity-20 shadow-xl"
                >
                ...
                <div className="flex bg-[var(--sp-surface)]/80 p-3 rounded-full border border-[var(--sp-border)] gap-3 overflow-x-auto no-scrollbar max-w-[50vw]">
                    {data.map((_, i) => (
                        <button
                            ...
                            className={`min-w-[50px] h-[50px] rounded-full text-sm font-black transition-all ${i === currentIndex ? 'bg-teal-500 text-slate-950 shadow-[0_0_20px_rgba(20,184,166,0.4)]' : 'text-[var(--sp-text-hi)]0/40 hover:text-[var(--sp-text-lo)]'}`}
                        >
    ...
    if (mode === 'immersive') {
        return (
            <div className="relative w-full min-h-screen bg-[var(--sp-bg)]">
                ...
                <ChandogyaImmersiveExperience lang={lang} onClose={() => setMode('classic')} />
            </div>
        );
    }

    return (
        <div className="min-h-screen bg-[#020606] text-white selection:bg-teal-500/30 selection:text-[var(--sp-text-hi)] font-sans overflow-x-hidden">

            {/* Hero Section */}
            <header className="relative h-[85vh] flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-b from-[var(--sp-gold)]/20 via-slate-950/60 to-[#020606]" />
                ...
                <div className="relative z-10 text-center px-6 max-w-7xl">
                    ...
                    >
                        <div className="inline-flex items-center gap-4 px-8 py-3 bg-teal-500/10 rounded-full border border-[var(--sp-border)] backdrop-blur-3xl">
                            <Compass className="w-5 h-5 text-[var(--sp-text-lo)] animate-spin-slow" />
                        ...
                        <div className="relative inline-block">
                            <h1 className="text-8xl md:text-[15rem] font-serif font-black text-transparent bg-clip-text bg-gradient-to-b from-[var(--sp-gold)] via-[var(--sp-surface)] to-[var(--sp-bg)] leading-[0.85] tracking-tighter">
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-center gap-12 pt-10">
                            <div className="flex flex-col items-center gap-2">
                                <span className="text-[var(--sp-text-hi)]0/40 text-[10px] font-black uppercase tracking-[0.3em]">Guru</span>
                                <span className="text-2xl font-serif italic text-[var(--sp-text-hi)]">Uddālaka Āruṇi</span>
                            </div>
                            ...
                            <div className="flex flex-col items-center gap-2">
                                ...
                                <span className="text-2xl font-serif italic text-[var(--sp-text-hi)]">Tat Tvam Asi</span>
                            </div>
                            <div className="w-px h-12 bg-teal-500/20 hidden md:block" />
                            <div className="flex flex-col items-center gap-2">
                                <span className="text-[var(--sp-text-hi)]0/40 text-[10px] font-black uppercase tracking-[0.3em]">Student</span>
                                <span className="text-2xl font-serif italic text-[var(--sp-text-hi)]">Śvetaketu</span>
                            </div>
                        </div>
                    </motion.div>
                </div>
            ...
            {/* Sticky Navigation */}
            <nav className="sticky top-0 z-50 bg-[#020606]/80 backdrop-blur-3xl border-b border-[var(--sp-border)] px-6 py-4">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        ...
                        <div className="hidden lg:block">
                            ...
                            <p className="text-[var(--sp-gold)] font-black text-[9px] uppercase tracking-[0.4em]">Ancient Song of Being</p>
                        </div>
                    </div>
                    ...
                    <div className="flex bg-[var(--sp-surface)]/60 rounded-[1.5rem] p-2 border border-white/5 shadow-2xl">
                        {tabs.map((tab) => (
                            <button
                                ...
                                className={`flex items-center gap-3 px-8 py-3 rounded-xl text-xs font-black transition-all ${activeTab === tab.id ? "bg-teal-500 shadow-[0_10px_20px_rgba(20,184,166,0.3)] text-slate-950" : "text-[var(--sp-gold)] hover:text-[var(--sp-text-lo)]"}`}
                            >
                        ...
                        <div className="hidden sm:flex bg-[var(--sp-surface)]/40 rounded-xl p-1.5 border border-white/5">
                            {(["en", "hi"] as const).map((l) => (
                                <button
                                    key={l}
                                    onClick={() => setLang(l)}
                                    className={`px-5 py-2 rounded-lg text-xs font-black transition-all ${lang === l ? "bg-teal-500 text-slate-950 shadow-lg" : "text-[var(--sp-gold)] hover:text-[var(--sp-text-lo)]"}`}
                                >
                                    {l.toUpperCase()}
                                </button>
                            ))}
            ...
            {/* Premium Footer */}
            <footer className="group relative bg-[#020606] py-48 text-center border-t border-[var(--sp-border)] overflow-hidden">
                ...
                <div className="relative z-10 space-y-12 max-w-4xl mx-auto px-6">
                    <div className="flex justify-center mb-4">
                        <InfinityIcon className="w-16 h-16 text-[var(--sp-gold)]/30 group-hover:text-[var(--sp-text-hi)]0 transition-colors duration-1000" />
                    </div>
                    <motion.p
                        whileHover={{ scale: 1.05 }}
                        className="text-5xl md:text-8xl font-serif text-[var(--sp-text-hi)] leading-tight transition-all group-hover:text-white group-hover:drop-shadow-[0_0_50px_rgba(20,184,166,0.3)]"
                    >
