...
export function SundayTestModal({ isOpen, onClose }: SundayTestModalProps) {
    ...
    const fetchTest = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000').replace(/\/$/, '')}/api/v1/antigravity/test/sunday`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await axios.post(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000').replace(/\/$/, '')}/api/v1/antigravity/test/submit`, payload);
            setResult(res.data);
            setSubmitted(true);
        } catch (error) {
        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                ...
                >
                        ...
                        {loading && !result ? (
                            ...
                            <div className="text-center py-12 px-6">
                                ...
                                <h3 className="text-5xl font-black text-white mb-2 tracking-tighter">
                                    {result.score} <span className="text-lg text-muted-foreground font-medium">/ {result.total_marks}</span>
                                </h3>
                                ...
                                <div className="flex justify-center items-center gap-6 mb-8">
                                    ...
                                    <div className="h-8 w-[1px] bg-card/10" />
                                    <div className="flex flex-col">
                                ...
                                {result.weak_topics && result.weak_topics.length > 0 && (
                                    <div className="mt-8 mb-10 p-6 bg-card/5 border border-white/10 rounded-[2rem] text-left">
                                ...
                                <div className="space-y-4">
                                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Negative Marking Applied: -0.66 per error</p>

                                    <button
                                        onClick={onClose}
                                        className="w-full py-4 bg-card text-black font-black uppercase tracking-widest rounded-2xl shadow-[0_10px_30px_rgba(255,255,255,0.1)] hover:bg-muted transition-all active:scale-95"
                                    >
                            ...
                            testData?.questions.map((q, index) => (
                                <div key={q.id} className="p-4 rounded-xl bg-card/5 border border-white/5">
                                    <div className="flex justify-between mb-2">
                                        <span className="text-xs font-mono text-muted-foreground">Q{index + 1} • {q.topic_tag}</span>
                                            ...
                                            {q.options.map((opt, i) => (
                                                <button
                                                    ...
                                                    className={`w-full text-left p-3 rounded-lg transition-all ${answers[q.id] === i
                                                        ...
                                                        : "bg-black/20 text-muted-foreground hover:bg-card/10"
                                                        }`}
                                                >
                    ...
                    {/* Footer */}
                    {!loading && !submitted && (
                        <div className="p-6 border-t border-white/10 bg-card/5 flex justify-end">
                            <button onClick={onClose} className="px-6 py-2 text-muted-foreground hover:text-white mr-4">
                                Cancel
                            </button>
                            <button
