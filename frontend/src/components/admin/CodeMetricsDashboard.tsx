            ...
            <Card className="border-t-4 border-t-indigo-600 shadow-xl overflow-hidden">
                ...
                <CardContent className="p-0">
                    <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x border-b border-border">
                        {/* LOC Counter */}
                        <div className="p-8 text-center group hover:bg-muted dark:hover:bg-slate-900/50 transition-colors">
                            <p className="text-xs text-muted-foreground mb-2 uppercase tracking-widest font-bold">Enterprise codebase size</p>
                        ...
                        {/* MCQ Counter */}
                        <div className="p-8 text-center group hover:bg-muted dark:hover:bg-slate-900/50 transition-colors">
                            <p className="text-xs text-muted-foreground mb-2 uppercase tracking-widest font-bold">Data volume (MCQs)</p>
                        ...
                        {/* Components Counter */}
                        <div className="p-8 text-center group hover:bg-muted dark:hover:bg-slate-900/50 transition-colors">
                            <p className="text-xs text-muted-foreground mb-2 uppercase tracking-widest font-bold">UI Modular architecture</p>
                                    ...
                                    <CardContent className="p-6">
                                        <div className="flex items-center justify-between mb-8">
                                            {metrics.files.map((file, idx) => (
                                                <div key={idx} className="text-center">
                                                    ...
                                                    <p className="text-[10px] font-black uppercase text-muted-foreground mb-1">{file.ext}</p>
                                                    <p className="text-sm font-bold text-muted-foreground dark:text-muted-foreground">{(file.value / 1000).toFixed(1)}k</p>
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div className="p-4 bg-card rounded-xl border shadow-sm">
                                            <div className="flex items-center justify-between mb-2">
                                                <p className="text-xs font-bold text-muted-foreground dark:text-muted-foreground">Scale Analysis</p>
    ...
    return (
        <div className="space-y-1">
            <div className="flex justify-between text-[11px] font-bold px-1 uppercase tracking-tight">
                ...
                <span className="text-indigo-600">{value.toLocaleString()} <span className="text-muted-foreground text-[10px]">({percentage.toFixed(1)}%)</span></span>
            </div>
            <div className="h-3 w-full bg-muted rounded-full overflow-hidden border border-border/50/50">
                <div
