...
function DraggableResource({ id, title }: { id: string; title: string }) {
    ...
    return (
        <div
            ...
            {...attributes}
            className={cn(
                "flex items-center gap-3 p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg cursor-grab active:cursor-grabbing shadow-sm hover:shadow-md transition-all touch-none",
    ...
    return (
        <div
            ...
            className={cn(
                ...
                isOver ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20" : "border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50"
            )}
        >
            <div className="flex items-start gap-3 mb-2">
                ...
                <div>
                    ...
                    <p className="text-xs text-slate-500">{linkedItems.length} resources linked</p>
                </div>
            </div>
            ...
            {linkedItems.length > 0 && (
                <div className="space-y-2 mt-auto">
                    {linkedItems.map(itemId => {
                        ...
                        return (
                            <div key={itemId} className="flex items-center gap-2 text-xs bg-white dark:bg-slate-800 p-2 rounded border border-slate-100 dark:border-slate-700">
            ...
            {linkedItems.length === 0 && !isOver && (
                <div className="mt-auto text-center py-2 text-xs text-slate-400">
        ...
        <DndContext onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            ...
            <DragOverlay>
                {activeId && activeResource ? (
                    <div className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 border-2 border-indigo-500 rounded-lg shadow-xl cursor-grabbing w-[250px] opacity-90 rotate-3">
                        <GripVertical className="h-4 w-4 text-slate-400" />
                        <div className="p-2 bg-red-100 rounded-md">
                            <FileText className="h-4 w-4 text-red-600" />
                        </div>
