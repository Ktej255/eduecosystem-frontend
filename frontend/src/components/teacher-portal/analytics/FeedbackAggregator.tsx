                    ...
                    <CardContent className="p-0 flex-1 overflow-y-auto max-h-[600px]">
                        <div className="divide-y divide-slate-100 dark:divide-slate-800">
                            {mockReviews.map((review) => (
                                <div key={review.id} className="p-6 hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors group">
                                    <div className="flex gap-4">
                                        ...
                                        <div className="flex-1 space-y-1">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <h4 className="font-semibold text-sm text-slate-900 dark:text-slate-100">{review.studentName}</h4>
                                                    <div className="flex items-center gap-2 mt-0.5">
                                                        <div className="flex text-amber-400">
                                                            {[...Array(5)].map((_, i) => (
                                                                <Star
                                                                    key={i}
                                                                    className={cn("h-3 w-3", i < review.rating ? "fill-current" : "text-slate-300 dark:text-slate-700")}
                                                                />
                                                            ))}
                                                        </div>
                                            ...
                                            <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed py-1">
                                                {review.comment}
                                            </p>

                                            <div className="flex items-center justify-between mt-2 pt-2">
                                                <Badge variant="secondary" className="text-[10px] text-slate-500 bg-slate-100 hover:bg-slate-200">
                                                    {review.course}
                                                </Badge>
