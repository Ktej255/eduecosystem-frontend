...
const ScrapbookContainer = ({ children }: { children: React.ReactNode }) => (
    <div className="bg-[#f0f4f8] min-h-screen p-4 md:p-8 font-['Kalam'] selection:bg-blue-100 selection:text-blue-900">
        <div className="max-w-5xl mx-auto space-y-12 bg-[url('https://www.transparenttextures.com/patterns/crumpled-paper.png')] bg-white rounded-3xl p-6 shadow-2xl border-4 border-blue-900 relative overflow-hidden">
...
const SecurityCard = ({ title, children, color = "border-blue-900", className = "" }: { title: string, children: React.ReactNode, color?: string, className?: string }) => (
    <div className={`bg-white border-2 ${color} rounded-2xl p-6 shadow-[4px_4px_0px_0px_rgba(30,64,175,0.1)] relative overflow-hidden group hover:-translate-y-1 transition-all duration-300 ${className}`}>
...
export default function NIAModule({ onComplete, isCompleted }: NIAModuleProps) {
    return (
        <ScrapbookContainer>
            {/* HERO SECTION */}
            <div className="relative bg-[#1e40af] border-4 border-blue-950 rounded-3xl p-8 md:p-12 shadow-[12px_12px_0px_0px_rgba(30,64,175,0.3)] overflow-hidden text-white">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-4">
                        <Badge className="bg-[#b91c1c] text-white px-4 py-1 text-lg border-2 border-white shadow-md">Chapter 58</Badge>
            ...
            <div className="grid md:grid-cols-1 gap-8">
                <SecurityCard title="Trial Mechanics" color="border-slate-900" className="bg-slate-50">
                    <div className="grid md:grid-cols-3 gap-6">
                        <div className="p-4 bg-white border-2 border-blue-900 rounded-2xl shadow-sm text-center">
                            <Gavel size={24} className="mx-auto mb-2 text-blue-900" />
                            <p className="text-[10px] font-black uppercase">Sessions Judge</p>
                            <p className="text-[8px] font-bold italic text-slate-400">Appointed on HC recommendation.</p>
                        </div>
                        <div className="p-4 bg-white border-2 border-blue-900 rounded-2xl shadow-sm text-center">
                            <Target size={24} className="mx-auto mb-2 text-red-600" />
                            <p className="text-[10px] font-black uppercase">Day-to-Day</p>
                            <p className="text-[8px] font-bold italic text-slate-400">Speedy trial on daily basis.</p>
                        </div>
                        <div className="p-4 bg-white border-2 border-blue-900 rounded-2xl shadow-sm text-center relative overflow-hidden">
                            ...
                            <p className="text-[8px] font-bold italic text-slate-400 underline underline-offset-2">Lies to High Court (Div Bench). NOT SC directly. [PYQ]</p>
                        </div>
                    </div>
                </SecurityCard>
            </div>
                ...
                >
                    {isCompleted ? (
                        ...
                        <span className="flex items-center gap-4">
                            ...
                            MARK CHAPTER 58 COMPLETE
                        </span>
                    )}
                </Button>
                <p className="mt-4 text-slate-400 font-bold italic tracking-widest uppercase text-xs">Sovereignty • Integrity • Security</p>
            </div>
        </ScrapbookContainer>
    );
}
