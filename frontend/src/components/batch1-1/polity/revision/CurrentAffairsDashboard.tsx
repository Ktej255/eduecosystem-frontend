...
import { Search, Filter, Calendar, BookOpen, ExternalLink, Flame } from 'lucide-react';
...
export default function CurrentAffairsDashboard() {
    ...
    const filteredAffairs = MAJOR_CURRENT_AFFAIRS.filter(item => {
                    ...
                    <Card key={item.id} className="hover:shadow-lg transition-all border-l-4 border-l-amber-500 overflow-hidden group">
                        <CardHeader className="pb-3 bg-slate-50">
                            <div className="flex justify-between items-start gap-4">
                                <div>
                                    <Badge variant="outline" className="mb-2 bg-white text-xs font-mono">
                                        {item.date}
                                    </Badge>
                        ...
                        <CardContent className="pt-4 space-y-4">
                            ...
                            <div className="space-y-2">
                                <div className="flex flex-wrap gap-1">
                                    {item.tags.map(tag => (
                                        <span key={tag} className="text-[10px] px-2 py-1 bg-slate-100 text-slate-600 rounded-full font-medium">
                                            #{tag}
                                        </span>
                                    ))}
                                </div>
                                <div className="flex flex-wrap gap-1 mt-2 pt-2 border-t border-slate-100">
                                    ...
                                    {item.topicIds.map(tid => (
                                        <Badge key={tid} variant="secondary" className="text-[10px] bg-blue-50 text-blue-700 hover:bg-blue-100 cursor-pointer">
                                            {tid}
                                        </Badge>
                                    ))}
                                </div>
