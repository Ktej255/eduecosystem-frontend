...
export default function RevisionDeepReports() {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-white dark:bg-slate-900/40 p-3 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm backdrop-blur-xl">
                            ...
                            <CardContent className="p-10">
                                ...
                                <div className="space-y-6">
                                    {strongTopics.length > 0 ? (
                                        ...
                                        <p className="text-slate-400 italic text-sm">No topic data available yet. Complete some cycles to see your performance.</p>
                                    )}
...
function StatCard({ icon: Icon, label, value, sub, color }: any) {
    return (
        <Card className="border-none bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-lg border border-slate-50 dark:border-slate-800">
            <CardContent className="p-10">
                ...
                <div className="text-4xl font-black mb-2 text-slate-900 dark:text-white tabular-nums tracking-tighter">{value}</div>
                <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">{label}</h4>
                <p className={`text-[10px] font-bold ${sub.includes('Up') ? 'text-emerald-500' : 'text-slate-400'}`}>{sub}</p>
            </CardContent>
        </Card>
    );
}
        ...
        <div className="space-y-3 p-4 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    <Badge variant="outline" className="font-black text-[10px] tracking-widest border-slate-200">{speed} SPEED</Badge>
                    <span className={`font-black text-lg text-${color}-600`}>{score}%</span>
                </div>
            </div>
            <Progress value={score} className="h-1.5 bg-slate-100 dark:bg-slate-800" />
        </div>
    );
}
...
function TopicProgressItem({ topic, score, trend, status = 'good' }: { topic: string; score: number; trend: 'improving' | 'stable' | 'declining'; status?: string }) {
    ...
    const trendColor = trend === 'improving' ? 'text-emerald-500' : trend === 'declining' ? 'text-rose-500' : 'text-slate-400';
