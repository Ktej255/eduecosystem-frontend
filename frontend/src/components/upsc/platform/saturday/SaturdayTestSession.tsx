...
import { ModuleMCQ, WeeklyTestConfig, calculateTestScore } from "../data/saturday-test-registry";
...
const SaturdayTestSession: React.FC<SaturdayTestSessionProps> = ({
    ...
    const questions = useMemo(() => config.subjects.flatMap(s => s.questions), [config]);
    const testTitle = config.title;

    const [currentIdx, setCurrentIdx] = useState(0);
    const [answers, setAnswers] = useState<Record<number, QuestionState>>({});
    const [timeLeft, setTimeLeft] = useState(config.totalDuration * 60);
    ...
    const handleFinish = useCallback(() => {
        ...
        const results = {
            ...calculateTestScore(config, new Map(Object.entries(answers).map(([id, s]) => [parseInt(id), s.selectedAnswer])), config.totalDuration * 60 - timeLeft),
            testTitle,
                        ...
                        <div className="flex justify-between items-start mb-6">
                            <div className="flex items-center gap-2">
                                ...
                                {currentQuestion.subject && (
                                    <span className="text-[10px] font-bold uppercase tracking-wider text-purple-400 px-2 py-1 bg-purple-400/10 rounded-md border border-purple-400/20 ml-auto">
                                        {currentQuestion.subject}
                                    </span>
                                )}
                            </div>
