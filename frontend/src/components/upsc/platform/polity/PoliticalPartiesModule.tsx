...
const PartyCard = ({ title, children, color = "bg-card" }: { title: string, children: React.ReactNode, color?: string }) => (
...
export default function PoliticalPartiesModule({ onComplete, isCompleted, chapterNumber = "79" }: PoliticalPartiesModuleProps) {
    return (
        <ScrapbookContainer>
            {/* HERO */}
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <PartyCard title="Legal Status (RPA, 1951)" color="bg-muted">
                    ...
                    <div className="mt-4 flex gap-4">
                        <div className="bg-card p-3 rounded border text-center flex-1">
                            <span className="block text-2xl font-black text-green-600">Recognized</span>
                            <span className="text-xs">Reserved Symbol</span>
                        </div>
                        <div className="bg-card p-3 rounded border text-center flex-1 opacity-60">
                            <span className="block text-2xl font-black text-muted-foreground">Registered</span>
                ...
                <PartyCard title="The Benefits" color="bg-blue-50">
                    <div className="space-y-3 font-bold text-sm text-muted-foreground">
            ...
            <div className="space-y-4">
                <div className="bg-card border-2 border-red-100 rounded-2xl p-6 shadow-md relative overflow-hidden">
                    ...
                    <div className="grid md:grid-cols-3 gap-6 text-center">
                        ...
                        <div className="p-4 bg-red-50 rounded-xl hover:bg-red-100 transition-colors">
                            ...
                            <p className="text-sm font-bold text-red-800">
                                2% of LS Seats (11) <br />
                                <span className="text-xs text-muted-foreground">from</span> <br />
                                3 Different States
                            </p>
                        </div>
                        <div className="p-4 bg-red-50 rounded-xl hover:bg-red-100 transition-colors">
                            ...
                            <p className="text-sm font-bold text-red-800">
                                ...
                                <span className="text-xs text-muted-foreground">in</span> <br />
                                4 States
                            </p>
                        </div>
                    </div>
                ...
                <div className="bg-slate-800 text-white rounded-2xl p-6 flex flex-wrap gap-4 items-center justify-center">
                    ...
                    {['BJP', 'INC', 'BSP', 'CPI(M)', 'NPP', 'AAP'].map(p => (
                        <Badge key={p} className="bg-card text-foreground font-black text-md px-3 py-1">
                            {p}
                        </Badge>
                    ))}
                </div>
            ...
            <div className="bg-green-50 rounded-2xl p-8 border-2 border-green-200">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 font-bold text-sm text-green-900 text-center">
                    <div className="bg-card p-3 rounded shadow-sm">6% Votes + 2 Assembly Seats</div>
                    <div className="bg-card p-3 rounded shadow-sm">6% Votes + 1 LS Seat</div>
                    <div className="bg-card p-3 rounded shadow-sm">3% of Assembly Seats</div>
                    <div className="bg-card p-3 rounded shadow-sm">1 LS Seat per 25</div>
                    <div className="bg-card p-3 rounded shadow-sm col-span-2 md:col-span-1 border-2 border-green-400">8% Total Votes (No Seats needed)</div>
                </div>
            </div>
