...
function CurrentAffairsCentralContent() {
    ...
    const handleBack = () => {
        ...
        if (initialSource === 'history_hub') {
            router.push('/student/upsc/history'); // Or whatever the history hub path is
    ...
    const handleReadArticle = (item: CurrentAffairItem) => {
        ...
        if (item.chapter && isHistory) {
            router.push(`/student/upsc/history/mcq?chapterId=${item.chapter}&level=3`);
        } else {
            // For now, maybe just show a toast or expand (placeholder)
            // router.push(`/student/upsc/current-affairs/${item.id}`);
        }
    };
                            ...
                            <div className="p-4 bg-muted border-t border-slate-100 flex justify-end">
                                {item.chapter && (item.subject.includes('History')) ? (
                                    ...
                                    <Button
                                        ...
                                        onClick={() => router.push(`/student/upsc/geography?view=lesson&topicId=${item.related_topic_id}`)}
                                    >
                                        <BookOpen className="w-3 h-3 mr-2" />
                                        Revise This Concept →
                                    </Button>
