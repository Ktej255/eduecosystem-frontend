...
export default function PerformingArtsMatrix() {
    ...
    return (
        <Card className="w-full bg-card dark:bg-[#111] border-border shadow-xl overflow-hidden h-[600px] flex flex-col">
            ...
            <div className="flex border-b border-border">
                <button
                    ...
                    className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest transition-all ${activeTab === 'dance' ? 'text-rose-600 border-b-2 border-rose-600 bg-rose-50/30' : 'text-muted-foreground'}`}
                >
                    Classical Dances
                </button>
                <button
                    onClick={() => { setActiveTab('music'); setSelectedItem(GHARANAS[0]); }}
                    className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest transition-all ${activeTab === 'music' ? 'text-rose-600 border-b-2 border-rose-600 bg-rose-50/30' : 'text-muted-foreground'}`}
                >
                    Music Gharanas
                </button>
            </div>
            ...
            <CardContent className="p-0 flex flex-1 overflow-hidden">
                {/* List Pane */}
                <div className="w-1/3 border-r border-border bg-slate-50/50 dark:bg-black/20 overflow-y-auto">
                    {(activeTab === 'dance' ? DANCE_FORMS : GHARANAS).map((item, i) => (
                        <div
                            key={i}
                            onClick={() => setSelectedItem(item)}
                            className={`p-4 border-b border-border cursor-pointer transition-all flex items-center justify-between group ${selectedItem?.name === item.name ? 'bg-card dark:bg-[#1a1a1a] border-l-4 border-l-rose-500 shadow-sm' : 'hover:bg-slate-100/50 dark:hover:bg-gray-800/30'}`}
                        >
                            <div className="flex flex-col">
                                <span className={`text-sm font-bold ${selectedItem?.name === item.name ? 'text-rose-600' : 'text-muted-foreground dark:text-muted-foreground'}`}>
                                    {item.name}
                                </span>
                                <span className="text-[10px] text-muted-foreground">{item.origin}</span>
                            </div>
                            <ChevronRight className={`w-4 h-4 transition-transform ${selectedItem?.name === item.name ? 'text-rose-500 translate-x-0' : 'text-muted-foreground opacity-0 group-hover:opacity-100 -translate-x-2'}`} />
                        </div>
                    ))}
                </div>

                {/* Detail Pane */}
                <div className="flex-1 bg-card dark:bg-[#0a0a0a] p-8 overflow-y-auto">
                        ...
                        >
                            <div className="flex items-start justify-between">
                                <div>
                                    ...
                                    <h2 className="text-3xl font-black text-foreground">{selectedItem.name}</h2>
                                </div>
                            ...
                            <div className="space-y-4">
                                <p className="text-sm text-muted-foreground dark:text-muted-foreground leading-relaxed italic border-l-4 border-border pl-4 py-1">
                            ...
                            <div className="pt-8 border-t border-border flex flex-col gap-4">
                                ...
                                {activeTab === 'dance' && (
                                    <Link href="/student/upsc/art-culture/21">
