...
export default function PYQExplorer() {
        ...
        if (newAnswers >= 5 && answeredCount > 0) {
            ...
            saveChapterReport(subjectKey === 'polity' || subjectKey === 'history' ? subjectKey : 'polity', 0, {
                ...
                questions: answeredQuestions.map((q, i) => ({
                    ...
                    chapter: 'PYQ',
                    subtopic: q.topic,
                    userAnswer: userAnswers[String(q.id)] ?? -1,
                    confidence: null,
                    timeSpent: 0
                }))
            }, 0); // Level 0 = PYQ
    ...
    return (
        <div className="space-y-4 md:space-y-8 w-full px-2 sm:px-4 md:px-8 lg:px-12 xl:px-16 pb-20">
            {/* Header Section */}
            <div className="relative overflow-hidden bg-muted dark:bg-black/40 border border-border dark:border-white/10 rounded-xl md:rounded-[2rem] backdrop-blur-3xl shadow-lg dark:shadow-2xl p-4 md:p-8 lg:p-10">
                ...
                <div className="relative flex flex-col gap-4 md:gap-6">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                            ...
                            <h2 className="text-2xl md:text-4xl lg:text-6xl font-black text-foreground tracking-tighter uppercase mb-1 md:mb-3">
                            ...
                            <p className="text-muted-foreground dark:text-muted-foreground text-sm md:text-base font-medium max-w-xl hidden md:block">
                                Access historical intelligence data. Analyze patterns from 2013-2024 to predict future trajectories.
                            </p>
                        </div>
                        <div className="flex items-center gap-3 bg-slate-200/50 dark:bg-card/5 px-4 md:px-5 py-2 md:py-2.5 rounded-full border border-border dark:border-white/10">
                            <Target size={16} className="text-emerald-600 dark:text-emerald-400" />
                            <span className="text-xs md:text-sm font-mono text-muted-foreground dark:text-muted-foreground">
                                <span className="text-foreground font-bold">{solvedCount}</span> / {subjectQuestions.length} Resolved
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="md:hidden">
                        <select
                            ...
                            className="w-full px-4 py-3 rounded-xl bg-card dark:bg-black/60 border border-border dark:border-white/10 text-foreground font-bold text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent appearance-none cursor-pointer"
                    ...
                    <div className="hidden md:flex flex-wrap gap-2 lg:gap-3 bg-card/60 dark:bg-black/60 p-3 lg:p-4 rounded-xl lg:rounded-2xl border border-border dark:border-white/10 backdrop-blur-md">
                        {Object.keys(subjectDataMap).map(sub => (
                            <button
                                key={sub}
                                onClick={() => { setActiveSubject(sub); setSelectedTopics([]); setSelectedYears([]); }}
                                className={`px-4 lg:px-6 py-2.5 lg:py-3 rounded-lg lg:rounded-xl text-sm lg:text-base font-bold uppercase transition-all duration-300 ${activeSubject === sub ? "bg-blue-600 text-white shadow-lg shadow-blue-500/25" : "text-muted-foreground hover:text-foreground dark:text-muted-foreground dark:hover:text-white hover:bg-black/5 dark:hover:bg-card/10"}`}
                            >
                                {sub}
                            </button>
                        ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div className="lg:col-span-3 xl:col-span-2 title-sticky lg:top-24 space-y-4">
                    <div className="bg-slate-100/50 dark:bg-black/30 border border-border dark:border-white/10 rounded-[1.5rem] backdrop-blur-xl overflow-hidden">
                        <div className="p-4 border-b border-border dark:border-white/5 flex justify-between items-center cursor-pointer" onClick={() => setShowFilters(!showFilters)}>
                            <div className="flex items-center gap-2">
                                <Filter size={14} className="text-blue-600 dark:text-blue-400" />
                                <span className="text-xs font-black uppercase tracking-widest text-foreground">Filter Intel</span>
                            </div>
                            <ChevronRight size={14} className={`text-muted-foreground transition-transform ${showFilters ? 'rotate-90' : ''}`} />
                        </div>
                                ...
                                >
                                    <div className="p-4 space-y-6">
                                        ...
                                        <div className="space-y-3">
                                            <span className="text-[10px] font-black text-muted-foreground dark:text-muted-foreground uppercase tracking-widest px-1">Timeline</span>
                                            <div className="flex flex-wrap gap-2">
                                                {allYears.map(year => (
                                                    <button
                                                        key={year}
                                                        onClick={() => toggleYear(year)}
                                                        className={`relative overflow-hidden px-2.5 py-1 rounded-md text-[10px] font-bold transition-all border ${selectedYears.includes(year) ? 'bg-blue-600 text-white border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.3)]' : 'bg-card/50 dark:bg-card/5 text-muted-foreground dark:text-muted-foreground border-border dark:border-white/5 hover:border-blue-400'}`}
                                                    >
                                                        {year}
                                                        <span className={`ml-1.5 text-[9px] opacity-70 ${selectedYears.includes(year) ? 'text-blue-100' : 'text-muted-foreground dark:text-muted-foreground'}`}>
                                                            {yearCounts[year]}
                                                        </span>
                                                    </button>
                                                ))}
                                        ...
                                        {/* Topics */}
                                        <div className="space-y-3">
                                            <span className="text-[10px] font-black text-muted-foreground dark:text-muted-foreground uppercase tracking-widest px-1">Classified Sectors</span>
                                            <div className="flex flex-col gap-1 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                                {allTopics.map(topic => (
                                                    <button
                                                        key={topic}
                                                        onClick={() => toggleTopic(topic)}
                                                        className={`w-full flex justify-between items-start text-left px-3 py-2 rounded-lg text-[11px] font-medium transition-all group ${selectedTopics.includes(topic) ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/30' : 'hover:bg-slate-200/50 dark:hover:bg-card/5 text-muted-foreground dark:text-muted-foreground border border-transparent'}`}
                                                    >
                                                        <span className="line-clamp-2">{topic}</span>
                                                        <span className={`px-1.5 py-0.5 rounded-sm text-[9px] font-bold bg-slate-200 dark:bg-black/20 shrink-0 ml-2 ${selectedTopics.includes(topic) ? 'text-amber-700 dark:text-amber-300' : 'text-muted-foreground dark:text-muted-foreground'}`}>
                                                            {topicCounts[topic]}
                                                        </span>
                                                    </button>
                                                ))}
                ...
                <div className="lg:col-span-9 xl:col-span-10 space-y-6">
                    ...
                    <div className="space-y-4">
                        {filteredPYQs.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-24 bg-card/[0.02] border border-dashed border-white/10 rounded-[2rem]">
                                <Lock size={48} className="text-muted-foreground mb-4" />
                                <p className="text-muted-foreground font-bold uppercase tracking-widest text-sm">No Intel Found</p>
                                <p className="text-muted-foreground text-xs mt-2">Adjust your clearance filters to access data.</p>
                            </div>
                        ) : (
        ...
        >
            ...
            <div className="flex flex-wrap justify-between items-center gap-4 mb-6 relative z-10">
                <div className="flex gap-2">
                    ...
                    <span className="px-3 py-1 rounded-full bg-card/5 border border-white/10 text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                        {pyq.topic}
                    </span>
                    {pyq.difficulty && (
            ...
            <div className="text-foreground font-medium text-lg leading-relaxed mb-8 relative z-10 font-sans space-y-3">
                <span className="text-muted-foreground dark:text-muted-foreground font-mono text-xs mr-2">Q{index + 1}.</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 relative z-10">
                {pyq.options.map((option, idx) => {
                    ...
                    if (!isAnswered) {
                        btnClass += " bg-card/50 dark:bg-card/[0.02] border-border dark:border-white/5 hover:bg-card dark:hover:bg-card/[0.05] hover:border-blue-400/50 dark:hover:border-white/20 hover:shadow-lg hover:shadow-blue-500/5";
                        markerClass += " bg-muted dark:bg-card/10 text-muted-foreground dark:text-muted-foreground group-hover/opt:bg-blue-600 group-hover/opt:text-white";
                    } else {
                        if (idx === pyq.correctIndex) {
                            ...
                            btnClass += " bg-muted dark:bg-black/20 border-border dark:border-white/5 opacity-50 grayscale";
                            markerClass += " bg-slate-200 dark:bg-black/40 text-muted-foreground dark:text-muted-foreground";
                        }
                    }
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className={`text-sm font-medium leading-snug ${isAnswered && idx === pyq.correctIndex ? 'text-emerald-700 dark:text-emerald-400 font-bold' : 'text-muted-foreground dark:text-muted-foreground group-hover/opt:text-blue-700 dark:group-hover/opt:text-white'}`}>
                                {option}
                            </span>
                        </button>
                    );
                    ...
                    >
                        <div className="p-1 rounded-[1.5rem] bg-gradient-to-br from-blue-500/20 via-blue-500/5 to-transparent">
                            <div className="bg-black/80 rounded-[1.4rem] p-6 border border-blue-500/10 relative">
                                ...
                                <div className="mt-6 text-sm text-muted-foreground leading-relaxed pl-4 border-l-2 border-blue-500/30 italic">
                                    {pyq.explanation}
                                </div>
                            </div>
                        </div>
