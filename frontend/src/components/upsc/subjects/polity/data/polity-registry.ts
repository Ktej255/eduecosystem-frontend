...
import { TOPIC_TITLES } from '../../../platform/polity/data/polity-types-95';
...
export function getTopicById(id: number): PolityTopic | undefined {
    ...
    const dynamicDef = TOPIC_TITLES.find((t: any) => t.id === id);
    if (dynamicDef) {
        return {
            id: dynamicDef.id,
...
export function getTopicsByModule(moduleId: string): PolityTopic[] {
    ...
    const dynamicTopics = TOPIC_TITLES
        .filter((t: any) => t.part === moduleId)
        .filter((t: any) => !POLITY_TOPICS.some((pt: any) => pt.id === t.id)) // Avoid duplicates
        .map((t: any) => getTopicById(t.id)!)
        .filter(Boolean);
