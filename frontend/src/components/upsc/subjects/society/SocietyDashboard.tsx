        ...
        <div className="max-w-7xl mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {SOCIETY_SYLLABUS.map((topic) => (
                    <Card key={topic.id} className="group hover:border-pink-300 transition-all hover:shadow-lg dark:bg-[#0a0a0a]">
                        <CardHeader className="pb-3">
                            ...
                            <CardTitle className="text-lg text-foreground group-hover:text-pink-600 dark:group-hover:text-pink-400 transition-colors">
                                {topic.title}
                            </CardTitle>
                        ...
                        <CardContent>
                            ...
                            <Link href={`/student/upsc/society/${topic.id}`}>
                                <button className="w-full py-2 bg-muted hover:bg-muted dark:hover:bg-slate-800 text-muted-foreground rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors border border-border group-hover:border-pink-200 dark:group-hover:border-pink-900/30">
...
function StatCard({ icon, label, value, color, bg }: { icon: React.ReactNode, label: string, value: string, color: string, bg: string }) {
    return (
        <div className={`p-5 rounded-2xl flex items-center gap-4 bg-card dark:bg-[#0a0a0a] border border-slate-100 shadow-sm`}>
