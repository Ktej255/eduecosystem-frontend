...
export default function MonitoringPage() {
    ...
    const handleStartExam = async () => {
        try {
            ...
            const res = await api.post("/monitoring/exam/start?exam_name=Midterm Physics")
    ...
    const handleViolation = async (type: string) => {
        if (!sessionId) return
        try {
            const res = await api.post(`/monitoring/exam/violation?session_id=${sessionId}&violation_type=${type}`)
            setViolations(res.data.total_violations)
            alert(`⚠️ VIOLATION DETECTED: ${type === "tab_switch" ? "Tab Switching" : "Exiting Fullscreen"}. This has been logged.`)
        } catch (error) {
            ...
            <div className="flex-1 space-y-8">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-gray-400 mt-2">Secure environment for high-stakes assessments.</p>
                    </div>
                </div>
                ...
                {!examStarted && !examFinished ? (
                    <div className="bg-gray-900 p-8 rounded-2xl border border-gray-800 text-center max-w-2xl mx-auto">
                        ...
                        <h2 className="text-2xl font-bold text-white mb-4">Ready for Midterm Physics?</h2>
                        <p className="text-gray-400 mb-8">
                            This exam is monitored. Switching tabs or exiting fullscreen will be logged as a violation.
                    ...
                    <div className="space-y-8">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="bg-red-900/20 border border-red-500/30 p-6 rounded-xl flex items-center">
                                ...
                                <div>
                                    <p className="text-red-400 text-sm font-bold uppercase">Violations</p>
                            ...
                            <div className="bg-green-900/20 border border-green-500/30 p-6 rounded-xl flex items-center">
                                ...
                                <div>
                                    <p className="text-green-400 text-sm font-bold uppercase">Status</p>
                                    <p className="text-xl font-bold text-white">Monitoring Active</p>
                                </div>
                            </div>
                            <div className="bg-blue-900/20 border border-blue-500/30 p-6 rounded-xl flex items-center">
                                ...
                                <div>
                                    <p className="text-blue-400 text-sm font-bold uppercase">Current Focus</p>
                                    <p className="text-xl font-bold text-white">{currentFocus}%</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white text-black p-12 rounded-xl min-h-[400px] shadow-2xl">
                            ...
                            <p className="text-lg mb-8">
                                Calculate the escape velocity of a projectile from the surface of the Earth.
                                Assume g = 9.8 m/s² and R = 6400 km.
                            </p>
                            <textarea
                    ...
                    <div className="bg-gray-900 p-8 rounded-2xl border border-gray-800 text-center max-w-2xl mx-auto">
                        ...
                        <div className="bg-gray-800 p-4 rounded-lg inline-block mb-6">
                            <p className="text-gray-300">Total Violations Recorded: <span className="text-white font-bold">{violations}</span></p>
                        </div>
                        <br />
                        <Button onClick={() => setExamFinished(false)} variant="outline">Return to Dashboard</Button>
                    </div>
                )}
            </div>
