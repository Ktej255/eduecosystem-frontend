...
export default function LMSPage() {
    ...
    const handleTakeQuiz = async () => {
        try {
            ...
            const transcript = "The Revolt of 1857 was a major uprising in India in 1857–58 against the rule of the British East India Company."
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="p-6 bg-gray-900 rounded-xl border border-gray-800 h-fit">
                    ...
                    <ul className="space-y-3 text-cyan-400 text-sm">
                        <li className="cursor-pointer hover:underline flex items-center"><span className="mr-2">📄</span> Download PDF Notes</li>
                        <li className="cursor-pointer hover:underline flex items-center"><span className="mr-2">🗺️</span> View Timeline Map</li>
                    </ul>
                </div>
            </div>
                        ...
                        {!quizFinished ? (
                            <>
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    <span className="text-gray-400">Q{currentQuestion + 1}/{quizData.questions.length}</span>
                                </div>
                                <div className="mb-8">
                                    <p className="text-xl text-white mb-6">{quizData.questions[currentQuestion].text}</p>
                                    <div className="grid grid-cols-1 gap-4">
                                        {JSON.parse(quizData.questions[currentQuestion].options).map((opt: string) => (
                                            <button
                                                key={opt}
                            ...
                            <div className="text-center py-8">
                                ...
                                <p className="text-xl text-gray-400 mb-8">You scored <span className="text-cyan-500 font-bold">{score}/{quizData.questions.length}</span></p>
                                <div className="flex justify-center space-x-4">
                                    <Button onClick={() => setShowQuiz(false)} variant="outline">Close</Button>
                                    <Button onClick={handleTakeQuiz} className="bg-cyan-600 hover:bg-cyan-500">Retry</Button>
                                </div>
                            </div>
                        )}
                    </div>
