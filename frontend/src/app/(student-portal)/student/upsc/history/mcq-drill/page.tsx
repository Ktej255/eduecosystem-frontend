...
import HistoryMCQSession from '@/components/upsc/subjects/history/HistoryMCQSession';
...
import StandardTestReport, { QuestionResult, TestResult } from '@/components/common/reports/StandardTestReport';
...
function MCQDrillContent() {
    ...
    useEffect(() => {
        async function fetchQuestions() {
            ...
            if (ids.length === 0 || (ids.length === 1 && ids[0] === 0)) {
                if (section === 'medieval') ids = Array.from({ length: 18 }, (_, i) => i + 1);
                else if (section === 'ancient') ids = Array.from({ length: 15 }, (_, i) => i + 1);
                else if (section === 'art_culture') ids = Array.from({ length: 15 }, (_, i) => i + 1);
                else ids = Array.from({ length: 39 }, (_, i) => i + 1);
            }
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-card dark:bg-black">
                ...
                <div className="mt-4 font-bold text-muted-foreground animate-pulse">Compiling {subject} Drill...</div>
            </div>
        );
    }
    ...
    if (questions.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen p-4">
                <div className="text-xl font-bold text-foreground">No questions found for {subject} chapters.</div>
                <button onClick={() => router.back()} className="mt-4 px-6 py-2 bg-indigo-600 text-white rounded-xl">Go Back</button>
            </div>
        );
    }
    ...
    if (showResults && testResult) {
        return (
            <StandardTestReport
                ...
                onBack={() => router.push('/student/upsc/history')}
            />
        );
    }
    ...
    const handleComplete = async (results: any[]) => {
        ...
        try {
            ...
            saveChapterReport(subject.toLowerCase() as any, primaryChapter, resultData, 10);
            ...
            <HistoryMCQSession
                ...
                onCancel={() => {
                    if (confirm("Are you sure you want to cancel? Progress will be lost.")) {
                        router.push('/student/upsc/history');
                    }
                }}
            />
        </div>
