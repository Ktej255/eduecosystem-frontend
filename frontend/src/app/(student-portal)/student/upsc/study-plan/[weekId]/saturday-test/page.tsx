...
import { getWeeklyTest } from "@/components/upsc/platform/data/saturday-test-registry";
import "@/components/upsc/platform/data/existing-polity-tests"; // Side effect: registers existing tests
...
export default function SaturdayTestPage({ params }: PageProps) {
    ...
    const weekData = getWeeklyTest(weekId);
    // Note: In the future, we might have multiple configs per week (Paper 1, Paper 2)
    // For now, we treat the single config as the main assessment.
    const questions = weekData?.subjects.flatMap(s => s.questions) || [];
    ...
    // Test view
    if (testState === 'paper1_test' && weekData) {
        return (
            <SaturdayTestSession
                config={weekData}
    ...
    // For Paper 2, we would need a separate config or a way to select paper 2 from the same week config.
    // For simplicity with the new framework, we assume one config per test session.
    if (testState === 'paper2_test' && weekData) {
        return (
            <SaturdayTestSession
                config={weekData}
