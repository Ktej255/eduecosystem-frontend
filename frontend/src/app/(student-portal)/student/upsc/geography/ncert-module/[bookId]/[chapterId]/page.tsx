...
export default function NCERTChapterPage({ params, searchParams }: { params: Promise<{ bookId: string; chapterId: string }>; searchParams: Promise<{ tab?: string }> }) {
    const router = useRouter();
    const { bookId, chapterId } = use(params);
    const sParams = use(searchParams);
    const initialTab = sParams.tab || "note";
