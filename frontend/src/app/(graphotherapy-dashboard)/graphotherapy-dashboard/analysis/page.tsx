    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            <div>
                ...
                <p className="text-muted-foreground mt-2">Select the depth of your scan.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Upload Section */}
                <Card className="h-full border-dashed border-2 bg-muted flex flex-col items-center justify-center p-12 text-center cursor-pointer hover:bg-muted transition-colors">
                    <div className="w-20 h-20 bg-card rounded-full shadow-sm flex items-center justify-center mb-6">
                        <UploadCloud className="w-10 h-10 text-muted-foreground" />
                    </div>
                    <h3 className="font-medium text-lg text-foreground">Upload Handwriting Sample</h3>
                ...
                <Card className="h-full border-0 shadow-lg">
                    <CardContent className="p-8 space-y-8">
                        <div>
                            <h3 className="font-bold text-foreground mb-4">Select Report Logic</h3>
                            <RadioGroup defaultValue="premium" onValueChange={setSelectedType} className="space-y-4">
                                {/* Premium Option */}
                                <div className={`flex items-center space-x-4 border p-4 rounded-xl transition-all ${selectedType === 'premium' ? 'border-purple-600 bg-purple-50' : 'border-border'}`}>
                                    <RadioGroupItem value="premium" id="r1" className="text-purple-600" />
                                    <Label htmlFor="r1" className="flex-1 cursor-pointer">
                                        <div className="font-bold text-foreground">Premium Blueprint</div>
                                        <div className="text-xs text-muted-foreground">7-Dimension Psychological Profile</div>
                                    </Label>
                                    <Badge className="bg-purple-100 text-purple-700 hover:bg-purple-100">Most Popular</Badge>
                                </div>

                                {/* Health Option (Locked/Unlocked) */}
                                <div className={`flex items-center space-x-4 border p-4 rounded-xl transition-all ${selectedType === 'health' ? 'border-red-600 bg-red-50' : 'border-border'}`}>
                                    ...
                                    <Label htmlFor="r2" className="flex-1 cursor-not-allowed opacity-60">
                                        ...
                                        <div className="text-xs text-muted-foreground">Requires Level 2 Completion</div>
                                    </Label>
                                </div>
                            </RadioGroup>
                        </div>
