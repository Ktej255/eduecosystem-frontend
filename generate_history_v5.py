...
def split_to_chips(text):
    ...
    chips = []
    for p in parts:
        # Further split by colon if it's like "Sources: Exploration..."
        subparts = p.split(':')
        for sp in subparts:
            s = sp.strip()
            if len(s) > 2:
    ...
    <div class="optional-layout">
        <div class="optional-main">
            <div class="paper-section">
                <h2 class="paper-title">Paper I: Historical Foundations</h2>
            ...
            <div class="paper-section">
                <h2 class="paper-title">Paper II: Modern India & World History</h2>
                <div class="accordion-container">
                    {p2_html}
                </div>
            </div>
            ...
            <div class="cta-bottom">
                <h3 class="mb-2" style="font-family: var(--font-head);">Redefine Your Preparation</h3>
                <p class="mb-4">Join 10,000+ aspirants using AI to master {subject_title} at Sarit Classes.</p>
                <a href="https://eduecosystem-frontend-yqncvzxdma-uc.a.run.app" class="btn-blue">Start Drill Session →</a>
            </div>
        </div>
...
def main():
    with open('history_full_data.json', 'r', encoding='utf-8') as f:
        history_data = json.load(f)
    
    output_dir = "d:/Development/EduEcosystem/Sarit Classes website code"
    content = generate_optional_page("optional-history", history_data)
    with open(os.path.join(output_dir, "optional-history.html"), 'w', encoding='utf-8') as f:
        f.write(content)
    print("Generated optional-history.html")
