...
class OrderService:
    ...
    def process_order(
        ...
        # Gather all required course IDs from direct items and bundles
        course_ids = set()
        bundle_ids = set()
        ...
        # Pre-fetch bundles and their courses
        bundles = {}
        if bundle_ids:
            bundles_list = db.query(CourseBundle).filter(CourseBundle.id.in_(bundle_ids)).all()
        ...
        if course_ids:
            ...
            existing_enrollments = {e[0] for e in enrollments}

        # Pre-fetch courses for updating enrollment counts
        courses_by_id = {}
        if course_ids:
            courses_list = db.query(Course).filter(Course.id.in_(course_ids)).all()
            courses_by_id = {c.id: c for c in courses_list}

        # Create enrollments for each course
        for item in order_items:
            if item.course_id:
                if item.course_id not in existing_enrollments:
                    ...
                    db.add(enrollment)
                    existing_enrollments.add(item.course_id)  # prevent duplicates within the same order
                ...
                bundle = bundles.get(item.bundle_id)
                if bundle and bundle.courses:
                    for course in bundle.courses:
                        if course.id not in existing_enrollments:
                            enrollment = Enrollment(
                                user_id=user_id,
                                course_id=course.id,
                                status="active",
                            ...
                            db.add(enrollment)
                            existing_enrollments.add(course.id)
