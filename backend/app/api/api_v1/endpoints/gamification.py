...
def get_shop_items(
    ...
    if not items:
        mock_items = [
            Reward(name="Neon Theme", cost=500, description="A glowing neon interface.", type="theme", image_url="neon_theme.png"),
            Reward(name="Golden Badge", cost=1000, description="Show off your wealth.", type="badge", image_url="gold_badge.png"),
            Reward(name="Double XP Potion", cost=200, description="2x XP for 1 hour.", type="consumable", image_url="potion.png"),
        ]
...
def purchase_item(
    ...
    if item.type != "consumable":
        existing = db.query(UserReward).filter(UserReward.user_id == current_user.id, UserReward.reward_id == item.id).first()
        if existing:
    ...
    return {"msg": "Purchase successful", "new_balance": current_user.coins, "item": item.name}
