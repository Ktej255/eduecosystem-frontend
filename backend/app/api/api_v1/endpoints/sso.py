...
def create_sso_config(
    ...
    config_dict = {
        ...
        "client_secret": (
            SSOService.encrypt_secret(config_data.client_secret)
...
async def initiate_sso_login(
    slug: str, request: Request, db: Session = Depends(deps.get_db)
):
    """
    ...
    if not org.sso_enabled:
        raise HTTPException(
            status_code=400, detail="SSO not enabled for this organization"
        )
...
async def handle_saml_callback(request: Request, db: Session = Depends(deps.get_db)):
    """
    ...
    if not config:
        ...
        config = (
            db.query(SSOConfig)
            .filter(SSOConfig.provider_type == SSOProviderType.SAML)
            .first()
        )
    ...
    saml_service = SAMLService(config, req_data)
    success, user_attrs, error = saml_service.process_response(
        {"SAMLResponse": saml_response}
    )

    if not success:
        raise HTTPException(
            status_code=400, detail=f"SAML authentication failed: {error}"
        )

    # JIT Provisioning
    try:
        user = SSOService.provision_user_from_sso(
            db, config.organization, user_attrs, config
        )
...
async def handle_oauth_callback(
    request: Request, code: str, state: str, db: Session = Depends(deps.get_db)
):
    """
    ...
    # Get user info
    success, user_info, error = await oauth_service.get_user_info(
        tokens["access_token"]
    )
    if not success:
