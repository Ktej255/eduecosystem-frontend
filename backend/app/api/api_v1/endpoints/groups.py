...
def join_pack(
    ...
    members = [GroupMember(full_name=m.full_name or "Unknown", streak_days=m.streak_days) for m in group.members]
    return GroupOut(id=group.id, name=group.name, description=group.description, members=members)
...
def get_my_group(
    ...
    avg_streak = sum(m.streak_days for m in group.members) / members_count if members_count > 0 else 0
