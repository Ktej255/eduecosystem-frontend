...
def init_db() -> None:
    ...
    try:
        ...
        if not db.query(ShadowModeSession).filter(ShadowModeSession.user_id == student.id).first():
        ...
        if not db.query(HandwritingSubmission).filter(HandwritingSubmission.user_id == student.id).first():
