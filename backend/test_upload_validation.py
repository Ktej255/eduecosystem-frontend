...
async def test_upload_validation():
    ...
    async with httpx.AsyncClient() as client:
        ...
        resp = await client.post(f"{BASE_URL}/login/access-token", data={"username": email, "password": password})
        ...
        resp = await client.post(f"{BASE_URL}/grapho/upload", files=files, headers=headers)
        if resp.status_code == 400 and "Invalid file type" in resp.text:
            print("✓ Correctly rejected invalid file type.")
        else:
            print(f"✗ Failed to reject invalid file type. Status: {resp.status_code}, Response: {resp.text}")
        ...
        if resp.status_code == 400 and "File too large" in resp.text:
            ...
            print(f"✗ Failed to reject large file. Status: {resp.status_code}, Response: {resp.text}")
        ...
        try:
            ...
            if resp.status_code == 200:
                print("✓ Valid file accepted.")
                 ...
                 print("✓ Valid file passed validation (failed at OCR as expected for fake content).")
            else:
                print(f"✗ Unexpected error for valid file. Status: {resp.status_code}, Response: {resp.text}")
