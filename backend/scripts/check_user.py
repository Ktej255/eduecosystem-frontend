...
def check_user(email):
    ...
    with engine.connect() as conn:
        result = conn.execute(text("SELECT id, email, full_name, is_batch1_authorized, is_batch2_authorized, hashed_password FROM users WHERE email = :email"), {"email": email}).fetchone()
        return result
