    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                # Register
                resp = await client.post(f"{BASE_URL}/users/", json={"email": email, "password": password, "full_name": "Rel Test"})
                if resp.status_code != 200:
            ...
            if resp.status_code == 200:
                group = resp.json()
                print(f"Joined group: {group.get('name')} (User -> Group relationship works).")
            else:
