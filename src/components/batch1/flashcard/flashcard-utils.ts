...
import { PolityTopic, Concept, PrelimsPointer } from '../polity/data/polity-types';
...
export function generateFlashcardsFromTopic(topic: PolityTopic): Flashcard[] {
    ...
    topic.coreArticles.forEach((article, idx) => {
        flashcards.push({
...
export function getFlashcardsForDay(cycleId: number, day: number): { topicIds: number[], totalTopics: number } {
    ...
    if (cycleId === 1) {
        // Special override for Day 2 to ensure Centre-State Relations (Topic 14) is shown
        if (day === 2) {
            return {
                topicIds: [14],
                totalTopics: 50
