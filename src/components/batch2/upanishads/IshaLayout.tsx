    ...
    return (
        <div className="space-y-8">
            <div className="text-center mb-10">
                <h2 className="text-3xl md:text-4xl font-serif font-bold text-amber-100 mb-2 drop-shadow-sm">{title}</h2>
                <p className="text-amber-600 font-medium tracking-widest uppercase text-xs">{subtitle}</p>
            </div>
            ...
            <motion.div
                ...
                className="bg-slate-900 rounded-3xl border border-amber-500/20 overflow-hidden shadow-2xl backdrop-blur-md"
            >
                <div className="grid grid-cols-1 lg:grid-cols-2">
                    ...
                    <div className="p-8 md:p-12 space-y-8">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-xl font-black text-slate-950 shadow-lg shadow-amber-500/20">
                                    {shloka.number}
                                </div>
                                <div>
                                    <h3 className="text-xl font-bold text-white">{shloka.theme}</h3>
                                    <p className="text-amber-600 text-xs font-bold uppercase tracking-widest">Isha Upanishad</p>
                                </div>
                            </div>
                        </div>

                        {/* Sanskrit Block */}
                        <div className="bg-slate-950/50 rounded-2xl p-6 border border-amber-500/10 shadow-inner">
                            ...
                            <p className="mt-4 text-amber-500/70 text-sm text-center italic leading-relaxed font-light">
                        ...
                        <div className="space-y-6">
                            <div className="bg-slate-900/50 rounded-xl p-5 border border-amber-500/5">
                            ...
                            {/* Sarit's Insight */}
                            <div className="bg-gradient-to-br from-amber-900/20 to-amber-800/10 rounded-2xl p-8 border border-amber-500/20 shadow-xl relative overflow-hidden group">
                                <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                                    <Sparkles className="w-12 h-12 text-amber-500" />
                                </div>
                                <h4 className="text-amber-200 font-black mb-4 flex items-center gap-3">
                                    <span className="bg-amber-500 text-slate-950 w-8 h-8 rounded-lg flex items-center justify-center text-lg shadow-lg">💡</span>
                    ...
                    {/* Visual Section */}
                    <div className="relative bg-slate-950 min-h-[500px] flex items-center justify-center overflow-hidden border-l border-amber-500/10">
                        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-amber-900/20 to-transparent pointer-events-none" />
                        ...
                        {image ? (
                            ...
                            <div className="text-center p-12 relative z-10">
                                <div className="text-8xl mb-6 opacity-30 animate-pulse text-amber-500">✨</div>
                                <p className="text-amber-500/50 font-medium italic">{shloka.nanoPrompt}</p>
                            </div>
                        )}

                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20">
                            <span className="px-4 py-1.5 bg-black/60 rounded-full text-[10px] text-amber-200 font-black tracking-[0.2em] uppercase border border-amber-500/30 backdrop-blur-md">
            ...
            <div className="flex flex-col items-center gap-6">
                <div className="flex items-center gap-4">
                    <button
                        ...
                        className="w-14 h-14 flex items-center justify-center bg-slate-900 text-amber-500 rounded-full border border-amber-500/20 hover:bg-amber-500 hover:text-slate-950 transition-all disabled:opacity-20 disabled:cursor-not-allowed group shadow-lg"
                    >
                        ...
                        {data.map((s, i) => (
                            <button
                                ...
                                className={`w-10 h-10 rounded-xl text-sm font-black transition-all border ${i === currentIndex
                                    ? "bg-amber-500 border-amber-400 text-slate-950 scale-110 shadow-lg shadow-amber-500/50"
                                    : "bg-slate-900 border-amber-500/10 text-amber-600 hover:border-amber-500/50 hover:text-amber-500"
                                    }`}
                            >
                                {s.number}
                            </button>
                ...
                <div className="flex items-center gap-3">
                    <div className="h-1 w-48 bg-slate-900 rounded-full overflow-hidden border border-amber-500/10">
                        <motion.div
...
// ==========================================
function IshaOverview({ lang, setActiveTab }: { lang: "en" | "hi", setActiveTab: (tab: any) => void }) {
    const pillars = [
        {
    ...
    return (
        <div className="space-y-16">
            <div className="text-center max-w-3xl mx-auto space-y-4">
                <h2 className="text-4xl md:text-5xl font-serif font-bold text-amber-100 italic drop-shadow-sm">
                    {lang === "en" ? "The Core Message" : "मुख्य संदेश"}
                </h2>
                <p className="text-amber-200/60 leading-relaxed text-lg">
                    {lang === "en"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {pillars.map((p, i) => (
                    <motion.div
                        ...
                        className="bg-slate-900 border border-amber-500/20 rounded-3xl p-8 hover:bg-slate-800 transition-all group relative overflow-hidden shadow-lg"
                    >
                        ...
                        <h3 className="text-xl font-bold text-amber-100 mb-4">{p.title}</h3>
            ...
            <div className="bg-gradient-to-br from-amber-900/10 to-transparent border border-amber-500/20 rounded-3xl p-10 mt-12 bg-slate-950">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                    <div className="space-y-6">
                        <h3 className="text-3xl font-serif font-bold text-amber-100 drop-shadow-md">
                        ...
                        <div className="space-y-4">
                            <div className="flex gap-4">
                                ...
                                <p className="text-amber-100/90 font-light">
                    ...
                    <div className="relative aspect-video rounded-2xl overflow-hidden bg-black flex items-center justify-center border border-amber-500/30 shadow-2xl">
                        <motion.div
                            animate={{ scale: [1, 1.1, 1], rotate: [0, 5, -5, 0] }}
                            transition={{ duration: 10, repeat: Infinity }}
                            className="text-8xl md:text-[120px] font-serif font-black text-transparent bg-clip-text bg-gradient-to-b from-amber-100 to-amber-700 opacity-10"
                        >
                            SO'HAM
                        </motion.div>
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                            <div className="w-32 h-32 rounded-full border border-amber-500 flex items-center justify-center mb-6 shadow-[0_0_30px_#f59e0b]">
                                <Sun className="w-16 h-16 text-amber-100 animate-pulse drop-shadow-[0_0_10px_#f59e0b]" />
                            </div>
                            <p className="text-amber-500 font-black tracking-widest text-xs uppercase shadow-black drop-shadow-md">The Final Truth</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-12">
            <div className="text-center">
                <h2 className="text-4xl md:text-5xl font-serif font-bold text-amber-100 mb-4 drop-shadow-md">The Golden Revelation</h2>
                <p className="text-amber-600 font-medium tracking-[0.3em] uppercase text-xs">Mantra 15 - 18: Beyond the Veil</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                {/* Visual Reveal Mechaninc (Lid) */}
                <div className="lg:col-span-12 relative aspect-[21/9] rounded-3xl overflow-hidden border-2 border-amber-500/30 shadow-2xl bg-black mb-12 shadow-amber-500/10">
                        ...
                        >
                            {image ? (
                                ...
                                <div className="text-center space-y-4">
                                    <div className="text-6xl animate-pulse text-amber-500">☀️</div>
                            ...
                            {isSoham && (
                                ...
                                >
                                    ...
                                    <p className="text-amber-500 font-black tracking-[0.5em] text-sm mt-4 uppercase">I AM THAT</p>
                                </motion.div>
                            )}
                        </motion.div>
                    </AnimatePresence>
                ...
                <div className="lg:col-span-12 space-y-8">
                    <AnimatePresence mode="wait">
                        ...
                        >
                            <div className="flex items-center gap-6">
                                <div className="w-16 h-16 rounded-2xl bg-amber-500 flex items-center justify-center text-3xl font-black text-slate-950 shadow-xl rotate-3 shadow-amber-500/30">
                                    {shloka.number}
                                </div>
                                <div>
                                    <h3 className="text-4xl font-serif font-bold text-amber-100 tracking-tight">{shloka.theme}</h3>
                                    <p className="text-amber-500 font-bold uppercase tracking-[0.2em] text-[10px]">Final Transcendence</p>
                                </div>
                            </div>

                            <div className="bg-slate-900 rounded-3xl p-8 border border-amber-500/20 backdrop-blur-sm space-y-6">
                            ...
                            <div className="bg-gradient-to-br from-amber-900/10 to-amber-800/5 rounded-3xl p-10 border border-amber-500/30 shadow-2xl relative overflow-hidden group">
                                <h4 className="text-amber-500 text-xs font-black uppercase tracking-[0.3em] mb-6 flex items-center gap-3">
                                    <Sparkles className="w-4 h-4" />
                                    Master Insight
                                </h4>
                                <p className="text-2xl text-amber-50 font-medium italic leading-relaxed relative z-10 transition-all group-hover:scale-[1.02]">
                                    {lang === "en" ? shloka.simpleExplanation : shloka.simpleExplanationHindi}
                                </p>
                            </div>
                        </motion.div>
                    ...
                    <div className="flex items-center justify-between pt-8 border-t border-amber-500/10">
                        <button
                            onClick={() => setCurrentIndex(prev => Math.max(0, prev - 1))}
                            disabled={currentIndex === 0}
                            className="flex items-center gap-3 px-8 py-4 bg-slate-900 text-amber-600 rounded-2xl border border-amber-500/20 hover:bg-amber-500 hover:text-slate-950 transition-all disabled:opacity-10"
                        >
                        ...
                        <div className="flex gap-3">
                            {prayerData.map((_, i) => (
                                <div
                                    key={i}
                                    className={`h-2 rounded-full transition-all duration-500 ${i === currentIndex ? "w-12 bg-amber-500" : "w-2 bg-slate-800"}`}
                                />
                            ))}
                        </div>
                        ...
                        <button
                            ...
                            className="flex items-center gap-3 px-8 py-4 bg-amber-500 text-slate-950 rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-amber-500/20 hover:bg-amber-400 transition-all disabled:opacity-10"
                        >
        ...
        <div className="min-h-screen bg-slate-950 text-amber-50 font-sans selection:bg-amber-500/30 selection:text-amber-200">
            ...
            <main className="max-w-7xl mx-auto px-6 py-16 md:py-24">
                <AnimatePresence mode="wait">
                    ...
                    >
                        {activeTab === "summary" && (
                            <IshaOverview lang={lang} setActiveTab={setActiveTab} />
                        )}
                        {activeTab === "research" && (
